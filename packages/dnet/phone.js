...
phone.getUserInfo = function(player, text) {
    ...
    mysql.executeQuery(`SELECT * FROM users WHERE id = '${methods.parseInt(text)}' OR name = '${methods.removeQuotes2(methods.removeQuotes(text))}'`, (err, rows, fields) => {
        ...
        rows.forEach(row => {
            ...
            {
                case 1:
                    regName = "Registracija";
                    break;
                case 2:
                    regName = "Lietuvos pilietybė";
                    break;
                default:
                    regName = "Nėra";
                    break;
            ...
            if (row['wanted_level'] > 0)
                wanted = 'Pavojaus lygis: ' + row['wanted_level'];
            ...
            items.push(phone.getMenuMainItem('Pagrindinis skyrius', subItems));

            subItems = [];

            let label = 'Nėra';
            if (row['a_lic']) {
                label = `Nuo ${row['a_lic_create']} iki ${row['a_lic_end']}`;
            }

            subItems.push(phone.getMenuItemButton(
                'А kategorijos licencija',
                label,
                { name: 'none' },
            ));
            ...
            if (row['b_lic'])
            {
                label = `Nuo ${row['b_lic_create']} iki ${row['b_lic_end']}`;
            }

            subItems.push(phone.getMenuItemButton(
                'B kategorijos licencija',
                label,
                { name: 'none' },
            ));
            ...
            if (row['c_lic'])
            {
                label = `Nuo ${row['c_lic_create']} iki ${row['c_lic_end']}`;
            }

            subItems.push(phone.getMenuItemButton(
                'C kategorijos licencija',
                label,
                { name: 'none' },
            ));
            ...
            if (row['air_lic'])
            {
                label = `Nuo ${row['air_lic_create']} iki ${row['air_lic_end']}`;
            }

            subItems.push(phone.getMenuItemButton(
                'Oro transporto licencija',
                label,
                { name: 'none' },
            ));
            ...
            if (row['ship_lic'])
            {
                label = `Nuo ${row['ship_lic_create']} iki ${row['ship_lic_end']}`;
            }

            subItems.push(phone.getMenuItemButton(
                'Vandens transporto licencija',
                label,
                { name: 'none' },
            ));
            ...
            if (row['taxi_lic'])
            {
                label = `Nuo ${row['taxi_lic_create']} iki ${row['taxi_lic_end']}`;
            }

            subItems.push(phone.getMenuItemButton(
                'Keleivių vežimo licencija',
                label,
                { name: 'none' },
            ));
            ...
            if (row['law_lic'])
            {
                label = `Nuo ${row['law_lic_create']} iki ${row['law_lic_end']}`;
            }

            subItems.push(phone.getMenuItemButton(
                'Teisininko licencija',
                label,
                { name: 'none' },
            ));
            ...
            if (row['biz_lic'])
            {
                label = `Nuo ${row['biz_lic_create']} iki ${row['biz_lic_end']}`;
            }
            ...
            if (row['fish_lic'])
            {
                label = `Nuo ${row['fish_lic_create']} iki ${row['fish_lic_end']}`;
            }
            ...
            if (row['med_lic'])
            {
                label = `Nuo ${row['med_lic_create']} iki ${row['med_lic_end']}`;
            }
            ...
            if (row['marg_lic'])
            {
                label = `Nuo ${row['marg_lic_create']} iki ${row['marg_lic_end']}`;
            }

            subItems.push(phone.getMenuItemButton(
                'Marihuanos vartojimo leidimas',
                label,
                { name: 'none' },
            ));

            items.push(phone.getMenuMainItem('Licencijos', subItems));

            phone.showMenu(player, 'userInfo', 'Duomenų bazė', items);
        });

        if (rows.length == 0) {
            let items = [];
            items.push(phone.getMenuMainItem(`Sąrašas tuščias`, [
                phone.getMenuItemButton(
                    `Paieškos metu nieko nerasta`,
                    ``
                )
            ]));
            phone.showMenu(player, 'userInfo', 'Duomenų bazė', items);
...
phone.getVehInfo = function(player, text) {
    ...
    mysql.executeQuery(`SELECT * FROM cars WHERE id = '${methods.parseInt(text)}' OR number = '${methods.removeQuotes2(methods.removeQuotes(text))}'`, (err, rows, fields) => {
        ...
        rows.forEach(row => {
            ...
            items.push(phone.getMenuMainItem('Transportas', subItems));
            phone.showMenu(player, 'vehInfo', 'Duomenų bazė', items);
        });
    ...
    mysql.executeQuery(`SELECT * FROM items WHERE params LIKE '%${text}%'`, (err, rows, fields) => {
        ...
        rows.forEach(row => {
            ...
            try {
                ...
                items.push(phone.getMenuMainItem('Ginklai', subItems));
            }
            catch (e) {
                
            }

            phone.showMenu(player, 'gunInfo', 'Duomenų bazė', items);
        });
...
phone.memberAction = function(player, id) {
    ...
    if (user.getId(player) == id) {
        player.notify('~r~Sis profilis nera prieinamas peržiurai');
        return;
    }
        ...
        rows.forEach(row => {
            ...
            if (!row['is_sub_leader']) {
                if (user.isLeader(player)) {
                    items.push(phone.getMenuItemModal(
                        'Išduoti pavaduotojo pareigybę',
                        '',
                        'Pavaduotojas',
                        `Ar tikrai norite išduoti pareigybę ${row['name']}?`,
                        'Išduoti',
                        'Atšaukti',
                ...
                items.push(phone.getMenuItemRadio(
                    'Keisti poziciją',
                    'Dabartinė padėtis: ' + fractionItem.rankList[row['rank_type1']][row['rank1']],
                ...
                if (user.isLeader(player) || user.isSubLeader(player)) {
                    ...
                    items.push(phone.getMenuItemRadio(
                        ...
                        'Dabartinis skyrius: ' + fractionItem.departmentList[row['rank_type1']],
                        'Pasirinkite skyrių',
                        depList,
                        { name: 'none' },
                        '',
                ...
                if (user.isLeader(player)) {
                    items.push(phone.getMenuItemModal(
                        'Nušalinkite pavaduotoją',
                        '',
                        'Pavaduotojas',
                        `Ar tikrai norite nušalinti ${row['name']}?`,
                        'Nušalinti',
                        'Atšaukti',
            ...
            if (user.isLeader(player) || user.isSubLeader(player)) {
                items.push(phone.getMenuItemModal(
                    ...
                    '',
                    'Atleisti',
                    `Ar tikrai norite atleisti ${row['name']}?`,
                    'Atleisti',
                    'Atšaukti',
            ...
            phone.showMenu(player, 'fraction', 'Veiksmai', [phone.getMenuMainItem('', items)]);
        });
    });
};
        ...
        rows.forEach(row => {
            ...
            if (!row['is_sub_leader2']) {
                if (user.isLeader2(player)) {
                    ...
                    items.push(phone.getMenuItemModal(
                        'Perduoti organizaciją',
                        '',
                        'Perdavimas',
                        `Ar tikrai norite perduoti organizaciją ${row['name']}?`,
                        'Perduoti',
                        'Atšaukti',
                ...
                items.push(phone.getMenuItemRadio(
                    'Keisti poziciją',
                    'Dabartinė padėtis: ' + fractionItemRanks[row['rank_type2']][row['rank2']],
                    'Pasirinkite poziciją',
                    rankList,
                    { name: 'none' },
                    '',
                ...
                if (user.isLeader2(player) || user.isSubLeader2(player)) {
                    ...
                    items.push(phone.getMenuItemRadio(
                        'Perkėlimas į kitą skyrių',
                        'Dabartinis skyrius: ' + fractionItemDep[row['rank_type2']],
                        'Pasirinkite skyrių',
                        depList,
                        { name: 'none' },
                        '',
            ...
            phone.showMenu(player, 'fraction2', 'Veiksmai', [phone.getMenuMainItem('', items)]);
        });
    });
};
    ...
    mysql.executeQuery(`SELECT id, social, name, family_id, rankf, rank_typef, is_sub_leaderf FROM users WHERE id = '${methods.parseInt(id)}'`, (err, rows, fields) => {
        ...
        rows.forEach(row => {
            ...
            if (!row['is_sub_leaderf']) {
                ...
                items.push(phone.getMenuItemRadio(
                    'Keisti poziciją',
                    'Dabartinė padėtis: ' + fractionItemRanks[row['rank_typef']][row['rankf']],
                    'Pasirinkti poziciją',
                    rankList,
                    { name: 'none' },
                    '',
                ...
                if (user.isLeaderF(player) || user.isSubLeaderF(player)) {
                    ...
                    items.push(phone.getMenuItemRadio(
                        'Perkelti į kitą skyrių',
                        'Dabartinis skyrius: ' + fractionItemDep[row['rank_typef']],
                        'Pasirinkite skyrių',
                        depList,
                        { name: 'none' },
                        '',
            ...
            phone.showMenu(player, 'family', 'Veiksmai', [phone.getMenuMainItem('', items)]);
        });
    });
};
...
phone.fractionVehicleAction = function(player, id) {
    ...
    if (user.isLeader(player) || user.isSubLeader(player)) {
        ...
        items.push(phone.getMenuItemRadio(
            'Perkelti į kitą skyrių',
            'Dabartinis skyrius: ' + fractionItem.departmentList[veh.rank_type],
            'Pasirinkite skyrių',
            depList,
            { name: 'none' },
            '',
        ...
        if (user.isLeader(player) && !veh.is_default) {
            items.push(phone.getMenuItemModal(
                'Parduoti',
                'Už: ' + methods.moneyFormat(methods.getVehicleInfo(veh.name).price / 2),
                'Pardavimas',
                `Ar tikrai norite parduoti ${veh.name}?`,
                'Parduoti',
                'Atšaukti',
    ...
    phone.showMenu(player, 'fraction', 'Veiksmai', [phone.getMenuMainItem(`${veh.name} | ${veh.number}`, items)]);
};
...
phone.fractionVehicleAction2 = function(player, id) {
    ...
    phone.showMenu(player, 'fraction2', 'Veiksmai', [phone.getMenuMainItem(`${veh.name} | ${veh.number}`, items)]);
};
...
phone.fractionMoney = function(player) {
    ...
    {
        items.push(phone.getMenuItemButton(
            `Ginklai: ${coffer.get(cofferId, 'stock_gun')}vnt.`,
            '',
        ));
        items.push(phone.getMenuItemButton(
            `Moduliai: ${coffer.get(cofferId, 'stock_gunm')}vnt.`,
            '',
        ));
        items.push(phone.getMenuItemButton(
            `Šaudmenys: ${coffer.get(cofferId, 'stock_ammo')}vnt.`,
            '',
        ));
        items.push(phone.getMenuItemButton(
            `Neperšaunamos liemenės: ${coffer.get(cofferId, 'stock_armour')}vnt.`,
            '',
        ));
        items.push(phone.getMenuItemButton(
            `Medicina: ${coffer.get(cofferId, 'stock_med')}vnt.`,
            '',
        ));
        items.push(phone.getMenuItemButton(
            `Maistas: ${coffer.get(cofferId, 'stock_eat')}vnt.`,
            '',
        ));
        items.push(phone.getMenuItemButton(
            `Kiti: ${coffer.get(cofferId, 'stock_other')}vnt.`,
            '',
        ));
    }
    ...
    if (user.isCartel(player)) {
        items.push(phone.getMenuItemButton(
            'Gauti furgoną su narkotikais',
            'Išlaidos: Medicina 500 vnt.',
            { name: 'getDrugVans' },
            '',
            true
        ));
    ...
    items.push(phone.getMenuItemModalInput(
        ...
        'Dabartinis užmokestis: ' + methods.moneyFormat(coffer.getBenefit(cofferId)),
        'Įveskite užmokestį',
    ...
    phone.showMenu(player, 'fraction', 'Biudžeto valdymas', [phone.getMenuMainItem(`Pagrindinis skyrius`, items)]);
};
...
phone.fractionLog = function(player) {
    ...
    mysql.executeQuery(`SELECT * FROM log_fraction WHERE fraction_id = ${fractionId} ORDER BY id DESC LIMIT 50`, (err, rows, fields) => {
        ...
        if (rows.length > 0) {
            try {
                let columns = [
                    { title: 'Numeris', field: 'id' },
                    { title: 'Vardas', field: 'name' },
                    { title: 'Aprašymas', field: 'text' },
                    { title: 'Suma', field: 'text2' },
                ...
                let item = phone.getMenuItemTable('Organizacijos istorija', columns, data);
        ...
        phone.showMenu(player, 'fractionHistory', `Organizacijos istorija`, items);
    });
};
...
phone.fractionLog2 = function(player) {
    ...
    mysql.executeQuery(`SELECT * FROM log_fraction_2 WHERE fraction_id = ${fractionId} ORDER BY id DESC LIMIT 50`, (err, rows, fields) => {
        ...
        phone.showMenu(player, 'fraction2History', `Organizacijos istorija`, items);
    });
};
...
phone.showGangList = function(player) {
    ...
    phone.showMenu(player, 'fraction2', `Gatvių sąrašas`, items);
};
...
phone.showCanabisList = function(player) {
    ...
    phone.showMenu(player, 'fraction2', `Teritorijų sąrašas`, items);
};
...
phone.fractionList = function(player) {
    ...
    mysql.executeQuery(`SELECT id, social, name, fraction_id, rank1, rank_type1, is_leader, is_sub_leader, is_online, login_date FROM users WHERE fraction_id = '${fractionId}' ORDER BY is_leader ASC, is_sub_leader ASC, rank_type1 ASC, is_online DESC, rank1 ASC, name ASC`, (err, rows, fields) => {
        ...
        rows.forEach(row => {
            ...
            if (row['is_leader']) {
                ...
                if (depPrev != row['rank_type1']) {
                    if (depList.length > 0)
                        items.push(phone.getMenuMainItem(`${depName} | ${depList.length} asm.`, depList));
        ...
        if (leaderItems.length > 0)
            newItems.push(phone.getMenuMainItem('Vadovas', leaderItems));

        phone.showMenu(player, 'fraction', `Narių sąrašas | ${rows.length} asm.`, newItems.concat(items));
    });
};
...
phone.fractionList2 = function(player, showStats = false) {
    ...
    mysql.executeQuery(`SELECT id, social, name, fraction_id2, rank2, rank_type2, is_leader2, is_sub_leader2, is_sub_leader2, st_order_atm_f, st_order_atm_d, st_order_drug_f, st_order_drug_d, st_order_lamar_f, st_order_lamar_d, login_date, is_online FROM users WHERE fraction_id2 = '${fractionId}' ORDER BY is_leader2 ASC, is_sub_leader2 ASC, rank_type2 ASC, is_online DESC, rank2 ASC, name ASC`, (err, rows, fields) => {
        ...
        rows.forEach(row => {
            ...
            try {
                ...
                if (row['is_online'] === 0)
                    desc = ` (${methods.unixTimeStampToDateTimeShort(row['login_date'])})`;
                if (showStats)
                    desc = ` (Z: ${row['st_order_drug_d']}/${row['st_order_drug_f']} | B: ${row['st_order_atm_d']}/${row['st_order_atm_f']} | L: ${row['st_order_lamar_d']}/${row['st_order_lamar_f']})`;
        ...
        try {
            ...
            phone.showMenu(player, 'fraction2', `Narių sąrašas | ${rows.length} asm.`, newItems.concat(items));
        }
        catch (e) {
            methods.debug(e);
        }
...
phone.fractionListF = function(player, showStats = false) {
    ...
    mysql.executeQuery(`SELECT id, social, name, family_id, rankf, rank_typef, is_leaderf, is_sub_leaderf, is_sub_leaderf, st_order_atm_f, st_order_atm_d, st_order_drug_f, st_order_drug_d, st_order_lamar_f, st_order_lamar_d, login_date, is_online FROM users WHERE family_id = '${fractionId}' ORDER BY is_leaderf ASC, is_sub_leaderf ASC, rank_typef ASC, is_online DESC, rankf ASC, name ASC`, (err, rows, fields) => {
        ...
        try {
            ...
            phone.showMenu(player, 'family', `Šeimos narių sąrašas | ${rows.length} asm.`, newItems.concat(items));
        }
        catch (e) {
            methods.debug(e);
        }
...
phone.fractionVehicles = function(player) {
    ...
    if (isLeader) {
        items.push(phone.getMenuMainItem(`Pagrindinis skyrius`, [
            phone.getMenuItemButton(
                'Transporto pirkimas',
                '',
                { name: 'vehicleBuyList' },
    ...
    mysql.executeQuery(`SELECT * FROM cars_fraction WHERE fraction_id = '${fractionId}' AND is_buy = '1' ORDER BY rank_type1 ASC, rank1 DESC, name ASC`, (err, rows, fields) => {
        rows.forEach(veh => {
            if (depPrev != veh['rank_type1']) {
                if (depList.length > 0)
                    items.push(phone.getMenuMainItem(`${depName} | ${depList.length} vnt.`, depList));
        ...
        phone.showMenu(player, 'fraction', `Organizacijos transporto priemonių parkas`, items);
    });
};
...
mysql.executeQuery(`SELECT * FROM cars_fraction WHERE fraction_id = '${fractionId * -1}' AND is_buy = '1' ORDER BY rank_type ASC, rank DESC, name ASC`, (err, rows, fields) => {
        rows.forEach(veh => {
            if (depPrev != veh['rank_type1']) {
                if (depList.length > 0)
                    items.push(phone.getMenuMainItem(`Transporto sąrašas`, depList));
                depList = [];
            }
        ...
        if (depList.length > 0)
            items.push(phone.getMenuMainItem(`Transporto sąrašas - ${depList.length} vnt.`, depList));

        phone.showMenu(player, 'fraction2', `Organizacijos transporto priemonių parkas`, items);
    });
};
...
phone.fractionVehiclesBuyList = function(player) {
    ...
    mysql.executeQuery(`SELECT * FROM cars_fraction WHERE fraction_id = '${fractionId}' AND is_buy = '0' ORDER BY name ASC, price ASC`, (err, rows, fields) => {
            ...
            rows.forEach(row => {
                ...
                if (name != row['name']) {
                    if (subItems.length > 0)
                        items.push(phone.getMenuMainItem(`${name} | ${subItems.length} vnt.`, subItems));
                    name = row['name'];
                    subItems = [];
                }
                ...
                let item = phone.getMenuItemTitle(
                    `${row['name']} | ${row['number']}`,
                    `Kaina: ${methods.moneyFormat(row['price'], 1)}`,
        ...
        phone.showMenu(player, 'fraction', `Transporto pirkimas`, items);
    });
};
...
phone.fractionVehiclesBuyList2 = function(player) {
    ...
    mysql.executeQuery(`SELECT * FROM cars_fraction WHERE fraction_id = '0' AND is_buy = '0' ORDER BY name ASC, price ASC`, (err, rows, fields) => {
        ...
        phone.showMenu(player, 'fraction2', `Transporto pirkimas`, items);
    });
};
...
phone.userAdList = function(player) {
    ...
    mysql.executeQuery(`SELECT * FROM rp_inv_ad ORDER BY id DESC LIMIT 30`, (err, rows, fields) => {
        ...
        phone.showMenu(player, 'adList', `Skelbimų sąrašas`, items);
    });
};
    ...
    mysql.executeQuery(`SELECT * FROM log_bank_user WHERE card = ${methods.parseInt(bankCard)} ORDER BY id DESC LIMIT 100`, (err, rows, fields) => {
        ...
        if (rows.length > 0) {
            try {
                ...
                let item = phone.getMenuItemTable('Bankinės operacijos', columns, data);
                items.push(phone.getMenuMainItem(``, [item]));
            }
            catch (e) {
        ...
        phone.showMenu(player, 'bankHistory', `Pervedimų istorija`, items);
    });
};
        ...
        mysql.executeQuery(`SELECT editor, count(*) as c FROM rp_inv_ad WHERE timestamp > ${(methods.getTimeStamp() - days * 86400)} GROUP BY (editor) ORDER BY c DESC`, (err, rows, fields) => {
            ...
            if (rows.length > 0) {
                try {
                    let columns = [
                        { title: '№', field: 'id' },
                        { title: 'Vardas', field: 'n' },
                        { title: 'Patikrinta', field: 'c' },
                    ];
                    let data = [];
                    ...
                    let item = phone.getMenuItemTable('Redagavimo įsipareigojimai', columns, data);
                    items.push(phone.getMenuMainItem(`Statistika per ${days}d.`, [item]));
                }
                catch (e) {
            ...
            phone.showMenu(player, 'statsad', `Statistika`, items);
        });
    }
    catch (e) {
        methods.debug(e);
    ...
    mysql.executeQuery(`SELECT * FROM log_player WHERE user_id = ${methods.parseInt(id)} ORDER BY id DESC LIMIT 100`, (err, rows, fields) => {
        ...
        if (rows.length > 0) {
            try {
                ...
                let item = phone.getMenuItemTable('Asmeninė istorija', columns, data);
                items.push(phone.getMenuMainItem(``, [item]));
            }
            catch (e) {
        ...
        phone.showMenu(player, 'userHistory', `Asmeninė istorija`, items);
    });
};
    ...
    mysql.executeQuery(`SELECT * FROM tickets WHERE user_id = ${methods.parseInt(id)} ORDER BY is_pay ASC, id DESC LIMIT 50`, (err, rows, fields) => {
        ...
        if (rows.length > 0) {
            try {
                let columns = [
                    { title: 'Nr', field: 'id' },
                    { title: 'Išdavė', field: 'do' },
                    { title: 'Atšaukė', field: 'do2' },
                    { title: 'Suma', field: 'price' },
                ...
                rows.forEach(row => {
                    data.push({
                        id: row['id'], do: row['do'], do2: row['do2'], price: methods.moneyFormat(row['price']), status: row['is_pay'] ? 'Sumokėta' : 'Nesumokėta', datetime: `${row['rp_datetime']} (( ${methods.unixTimeStampToDateTimeShort(row['timestamp'])} ))`
                    });
                });

                let item = phone.getMenuItemTable('Baudų istorija', columns, data);
                items.push(phone.getMenuMainItem(``, [item]));
            }
            catch (e) {
        ...
        phone.showMenu(player, 'userTickets', `Baudų istorija`, items);
    });
};
...
phone.createFraction = function(player) {
    ...
    mysql.executeQuery(`SELECT * FROM fraction_list WHERE owner_id = 0`, (err, rows, fields) => {
        ...
        if (rows.length > 0) {
            ...
            items.push(phone.getMenuMainItem(`Laisvų slotų sąrašas`, subItems));
        }
        else {
            items.push(phone.getMenuMainItem(`Sąrašas tuščias`, [
                phone.getMenuItemButton(
...
phone.fractionAll = function(player) {
    ...
    mysql.executeQuery(`SELECT * FROM fraction_list WHERE owner_id > 0`, (err, rows, fields) => {
        ...
        if (rows.length > 0) {
            ...
            items.push(phone.getMenuMainItem(`Organizacijų sąrašas`, subItems));
        }
        else {
            items.push(phone.getMenuMainItem(`Sąrašas tuščias`, [
                phone.getMenuItemButton(
    ...
    mysql.executeQuery(`SELECT * FROM cars WHERE user_id = '${user.getId(player)}' ORDER BY name ASC`, (err, rows, fields) => {
        ...
        if (rows.length > 0) {
            ...
            rows.forEach(row => {
                ...
                let item = phone.getMenuItemTitle(
                    `${row['name']}`,
                    `Valstybinė kaina: ${methods.moneyFormat(row['price'], 1)}`,
                    { name: 'none' },
                    enums.getVehicleImg(row['name'])
                );
                subItems.push(item);
                ...
                item = phone.getMenuItemButton(
                    'Iškvieskite vilkiką',
                    'Kaina: ' + methods.moneyFormat(price),
                ...
                {
                    item = phone.getMenuItemButton(
                        'Atrakinti / Užrakinti duris',
                        'Nuotolinis transporto priemonės valdymas',
                        { name: "lock", id: row['id'] },
                        '',
                        true,
                    );
...
phone.fractionVehicleBuyInfo = function(player, id) {
    ...
    mysql.executeQuery(`SELECT * FROM cars_fraction WHERE id = '${id}'`, (err, rows, fields) => {
        ...
        rows.forEach(row => {
            ...
            item = phone.getMenuItemButton(
                'Priežiūros išlaidos',
                `${methods.moneyFormat(vehicles.getFractionDay(vInfo.price))} savaitei (( dienai ))`,
            );
            subItems.push(item);

            item = phone.getMenuItemModal(
                'Pirkti',
                `Kaina: ${methods.moneyFormat(vInfo.price)}`,
                'Pirkimas',
                `Ar tikrai norite pirkti ${vInfo.display_name}?`,
                'Pirkti',
                'Atšaukti',
...
phone.deleteChat = function(player, phoneNumber) {
    ...
    player.notify('~b~Pokalbis buvo istrintas');
};
    ...
    try {
        ...
        mp.players.forEach(p => {
            if (user.isLogin(p) && user.get(p, 'phone_type') > 0 && user.get(p, 'phone') === methods.parseInt(phoneNumber)) {
                user.sendPhoneNotify(p, methods.phoneFormat(numberFrom), '~b~Nauja žinutė', message);
...
phone.getMenuItemModal = function(title, text, modalTitle, modalText, modalYes = 'Taikyti', modalNo = 'Atšaukti', params = { name: "null" }, img = undefined, clickable = false, background = undefined) {
    return {
        title: title,
        text: text,
        type: 7,
...
phone.getMenuItemModalInput = function(title, text, modalTitle, modalValue = '', modalYes = 'Taikyti', modalNo = 'Atšaukti', params = { name: "null" }, img = undefined, clickable = false, background = undefined) {
    return {
        title: title,
        text: text,
        type: 8,

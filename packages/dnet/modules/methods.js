    ...
    mp.players.forEach(async (p) => {
        if (user.isLogin(p)) {
            try {
                ...
                if (!isSave) {
                    p.notify('~r~Paskyros issaugojimo klaida, praneskite serverio kurejams');
                    p.notify('~r~Butinai prisiminkite laika');
...
methods.loadAllBlips = function () {
    ...
    methods.createBlip(new mp.Vector3(536.4715576171875, -3126.484375, 5.073556900024414), 598, 0, 0.8, 'Armija');
    ...
    methods.createBlip(new mp.Vector3(-158.44952392578125, -605.221923828125, 48.23460388183594), 535, 67, 0.8, 'Verslo centras');
    //methods.createBlip(new mp.Vector3(111.5687, -749.9395, 30.69), 498, 0, 0.8, 'Здание FIB');
    methods.createBlip(new mp.Vector3(1830.489, 2603.093, 45.8891), 238, 0, 0.8, 'Kalejimas');
    methods.createBlip(new mp.Vector3(-1290.544, -571.1852, 29.57288), 419, 0, 1, 'Vyriausybe');
    methods.createBlip(new mp.Vector3(5011.5771484375, -5750.75830078125, 31.852725982666016), 304, 5, 1, 'Vyriausybe');

    methods.createBlip(new mp.Vector3(311.9224853515625, -583.9681396484375, 44.299190521240234), 489, 59, 0.8, 'Ligonine');
    methods.createBlip(new mp.Vector3(-253.9735565185547, 6320.83935546875, 37.61736297607422), 489, 59, 0.8, 'Ligonine');
    methods.createBlip(new mp.Vector3(1836.8359375, 3676.784912109375, 33.27006912231445), 489, 59, 0.8, 'Ligonine');
    methods.createBlip(new mp.Vector3(4962.66845703125, -5791.33447265625, 25.266326904296875), 489, 59, 0.8, 'Ligonine');

    methods.createBlip(new mp.Vector3(-759.5448608398438, -709.0863037109375, 29.0616512298584), 305, 60, 0.6, 'Baznycia');
    ...
    methods.createBlip(new mp.Vector3(1200.6854248046875, -1474.0303955078125, 33.85951614379883), 436, 60, 0.8, 'Gaisrine');
    methods.createBlip(new mp.Vector3(-369.2587890625, 6114.552734375, 30.446975708007812), 436, 60, 0.8, 'Gaisrine');

    methods.createBlip(new mp.Vector3(-1654.792236328125, -948.4613037109375, 7.716407775878906), 225, 2, 0.8, 'Automobiliu turgus');
    methods.createBlip(new mp.Vector3(903.91162109375, -165.71511840820312, 73.09003448486328), 198, 5, 0.8, 'Taksi parkas');

    methods.createBlip(pickups.BotSellGun, 728, 5, 0.6, 'Liejykla');
    methods.createBlip(pickups.BotSellCloth, 728, 3, 0.6, 'Drabužiu fabrikas');

    methods.createBlip(pickups.BotSellGun1, 728, 5, 0.6, 'Liejykla');
    methods.createBlip(pickups.BotSellCloth1, 728, 3, 0.6, 'Drabužiu fabrikas');

    methods.createBlip(pickups.IslandPos1, 404, 0, 0.8, 'Perplaukimas');
    methods.createBlip(pickups.IslandPos2, 404, 0, 0.8, 'Perplaukimas');

    methods.createBlip(new mp.Vector3(742.2985229492188, -925.4149780273438, 23.978235244750977), 728, 39, 0.6, 'Juodasis turgus');
    methods.createBlip(new mp.Vector3(-1685.0235595703125, -763.0075073242188, 9.18997859954834), 728, 0, 0.6, 'Turgus');

...
mp.events.addRemoteCounted('server:duel:toLobby', (player, targetId, bet, death) => {
    try {
        ...
        if (mp.players.exists(target)) {
            ...
            if (bet > 25000) {
                player.notify('~r~Statymas negali buti didesnis nei 25000€');
                return;
            }
            if (bet + 250 > user.getCashMoney(player)) {
                player.notify('~r~Jus neturite tokios sumos');
                return;
            }
            if (bet > user.getCashMoney(target)) {
                player.notify('~r~Žaidejas neturi tokios sumos');
                return;
            }
            if (user.get(player, 'rating_duel_mmr') > user.get(target, 'rating_duel_mmr') + 900 || user.get(target, 'rating_duel_mmr') > user.get(player, 'rating_duel_mmr') + 900) {
                player.notify('~r~Jusu ivertinimas yra per daug skirtingas');
                return;
            }
    ...
    try {
        ...
        if (mp.players.exists(target)) {
            try {
                ...
                user.removeCashMoney(target, 250, 'Dvikova su žaidėju ' + user.getRpName(player));
                user.removeCashMoney(player, 250, 'Dvikova ' + user.getRpName(target));
                ...
                if (bet > 0) {
                    user.removeCashMoney(player, bet, 'Dvikovos statymas');
                    user.removeCashMoney(target, bet, 'Dvikovos statymas');
                }
    ...
    try {
        ...
        mysql.executeQuery(`SELECT name, rating_racer_mmr, rating_racer_count, rating_racer_win FROM users ORDER BY rating_racer_mmr DESC LIMIT 10`, function (err, rows, fields) {
            try {
                ...
                menuData.set('~s~...', ``);
                menuData.set('~s~Jūsų statistika', `~g~${user.get(player, 'rating_racer_mmr')}~s~ | ~q~${user.get(player, 'rating_racer_count')}~s~ | ~y~${user.get(player, 'rating_racer_win')}`);
                user.showMenu(player, 'Reitingas', '~g~MMR~s~ | ~q~Atvykimai~s~ | ~y~Pergalės', menuData);
            }
            catch (e) {
                methods.debug(e);
            }
    ...
    try {
        ...
        mysql.executeQuery(`SELECT name, rating_duel_mmr, rating_duel_count, rating_duel_win FROM users ORDER BY rating_duel_mmr DESC LIMIT 10`, function (err, rows, fields) {
            try {
                ...
                menuData.set('~s~...', ``);
                menuData.set('~s~Jūsų statistika', `~g~${user.get(player, 'rating_duel_mmr')}~s~ | ~q~${user.get(player, 'rating_duel_count')}~s~ | ~y~${user.get(player, 'rating_duel_win')}`);
                user.showMenu(player, 'Reitingas', '~g~MMR~s~ | ~q~Dvikovos~s~ | ~y~Pergalės', menuData);
            }
            catch (e) {
                methods.debug(e);
            }
...
mp.events.addRemoteCounted('server:user:toLspdSafe', (player) => {
    try {
        ...
        if (user.get(player, 'online_lspd') > 1) {
            user.showCustomNotify(player, `Jums liko pražaisti ${methods.parseFloat(user.get(player, 'online_lspd') * 8.5 / 60).toFixed(2)}val del prieigos prie konfiskuotu prekiu`);
            return;
        }
...
mp.events.addRemoteCounted('server:cont:vehicle', (player, veh) => {
    ...
    try {
        if (user.get(player, 'online_time') < 70) {
            player.notify(`~r~Jums liko pražaisti ${methods.parseFloat((70 - user.get(player, 'online_time')) * 8.5 / 60).toFixed(1)}`);
            return;
        }
        if (user.get(player, 'is_take_vehicle')) {
            player.notify('~r~Jus jau turite transporta');
            return;
        }
        ...
        player.notify('~g~Jus gavote transporta');
    }
    catch (e) {}
});
...
mp.events.addRemoteCounted('server:cont:vehicle2', (player, veh) => {
    ...
    try {
        if (user.get(player, 'online_contall') < 494) {
            player.notify(`~r~Jums liko pražaisti ${methods.parseFloat((494 - user.get(player, 'online_contall')) * 8.5 / 60).toFixed(1)}`);
            return;
        }
        if (user.get(player, 'is_take_vehicle_2')) {
            player.notify('~r~Jus jau gavote transporta');
            return;
        }
...
mp.events.addRemoteCounted('server:user:warnAntiCheat', (player, reason) => {
    chat.sendToAdmin(`Įtarimas sukčiavimu ${user.getRpName(player)} (${player.id})`, reason);
});
...
mp.events.addRemoteCounted('server:user:giveTicket', (player, id, price, reason) => {
    ...
    let desc = `${user.getRpName(player)} (${user.getFractionName(player)} | ${user.getDepartmentName(player)}) | Priežastis: ${reason}`;
    let target = user.getPlayerById(id);
    if (user.isLogin(target)) {
        player.notify(`~r~Jums buvo paskirta bauda, kurios dydis ${price}\n${reason}`);
        if (target.id === player.id)
            desc = 'Sistema | Priežastis: ' + reason;
...
mp.events.addRemoteCounted('server:user:payTicket', (player, id, price) => {
    ...
    if (user.getBankMoney(player) < price) {
        player.notify('~r~Korteleje neturite pinigu baudai sumoketi.');
        return;
    }
    user.removeBankMoney(player, player, 'Baudos mokėjimas #' + id);
    mysql.executeQuery(`UPDATE tickets SET is_pay='1' WHERE id='${id}'`);
    player.notify('~g~Jus sumokejote bauda!');
});
...
mp.events.addRemoteCounted('server:vehicle:selltojunkyard', (player, price) => {
    ...
    if (remotePlayer && user.isLogin(remotePlayer)) {
        try {
            if (user.isCuff(remotePlayer) || user.isTie(remotePlayer)) {
                player.notify('~r~Žaidejas surakintas');
                return;
            }
            ...
            if (remotePlayer.id != player.id) {
                ...
                chat.sendMeCommand(remotePlayer, 'pažiūrėjo dokumentus');
                chat.sendMeCommand(player, 'parodė dokumentus');
            }
            else
                chat.sendMeCommand(player, 'pažiūrėjo dokumentus');

            remotePlayer.notify('~b~Dokumentus galite paslepti paspausdami mygtuka ~s~BACKSPACE ~b~arba ~s~ESC');
            ...
            if (lic == 'card_id') {
                ...
                if (user.get(player, 'work_lic') != '') {
                    ...
                    player.notify('~r~Jums truksta Work ID');
                    if (remotePlayer.id != player.id)
                        remotePlayer.notify('~r~Truksta žaidėjo Work ID');
                }
            }
            else {
                let licName = '';
                ...
                switch (lic) {
                    case 'a_lic':
                        licName = 'A kategorija';
                        licPref = 'A';
                        break;
                    case 'b_lic':
                        licName = 'B kategorija';
                        licPref = 'B';
                        break;
                    case 'c_lic':
                        licName = 'C kategorija';
                        licPref = 'C';
                        break;
                    case 'air_lic':
                        licName = 'Oro transportas';
                        licPref = 'P';
                        break;
                    case 'ship_lic':
                        licName = 'Vandens transportas';
                        licPref = 'S';
                        break;
                    case 'taxi_lic':
                        licName = 'Keleivių vežimas';
                        licPref = 'T';
                        break;
                    case 'law_lic':
                        licName = 'Teisininko';
                    ...
                    case 'fish_lic':
                        ...
                        break;
                    case 'med_lic':
                        licName = 'Sveikatos draudimas';
                        licPref = 'M';
                        break;
                    case 'marg_lic':
                        licName = 'Marihuanos vartojimo';
                        licPref = 'M';
                        break;
                }
                ...
                if (user.get(player, lic)) {
                    ...
                    player.notify('~r~Neturite licencijos tipo: ~s~' + licName);
                    if (remotePlayer.id != player.id)
                        remotePlayer.notify('~r~Žaidejas neturi licencijos tipo: ~s~' + licName);
                }
            }
        }
        catch (e) {
    ...
    if (remotePlayer && user.isLogin(remotePlayer)) {
        try {
            ...
            if (remotePlayer.id != player.id) {
                user.playAnimation(remotePlayer, "mp_common","givetake2_a", 8);
                user.playAnimation(player, "mp_common","givetake1_a", 8);
                chat.sendMeCommand(remotePlayer, 'pažiūrėjo tapatybės kortelę');
                chat.sendMeCommand(player, 'parodė tapatybės kortelę');
            }
            else
                chat.sendMeCommand(player, 'pažiūrėjo tapatybės kortelę');
...
mp.events.addRemoteCounted('server:user:unCuffById', (player, targetId) => {
    ...
    if (mp.players.exists(target)) {
        ...
        if (!user.isPolice(player) && !user.isGov(player) && !user.isCartel(player)) {
            player.notify('~r~Jums neleidžiama to daryti');
            return;
        }
        ...
        if (user.isCuff(target)) {
            ...
            chat.sendMeCommand(player, 'nuėmė antrankius');
        }
...
mp.events.addRemoteCounted('server:user:cuffItemById', (player, targetId) => {
    ...
    if (mp.players.exists(target)) {
        ...
        mysql.executeQuery('SELECT id FROM items WHERE item_id = 40 AND owner_type = 1 AND owner_id = ' + user.getId(player), function (err, rows, fields) {
            if (rows.length === 0) {
                player.notify('~r~Jus neturite antrankiu');
                return;
            }
...
mp.events.addRemoteCounted('server:user:unTieById', (player, targetId) => {
    ...
    if (mp.players.exists(target)) {
        ...
        if (user.isTie(target)) {
            ...
            chat.sendMeCommand(player, 'nuėmė raiščius');
        }
...
mp.events.addRemoteCounted('server:user:knockById', (player, targetId) => { //TODO
    ...
    {
        player.notify("~r~Laiko limitas 2 minutes siam veiksmui");
        return;
    }
    ...
    if (mp.players.exists(target)) {
        ...
        {
            player.notify("~r~Sis veiksmas kalejime negalimas");
            return;
        }
        if (user.get(player, 'med_time') > 0 || user.get(target, 'med_time') > 0)
        {
            player.notify("~r~Sis veiksmas ligonineje negalimas");
            return;
        }

        if (user.get(player, 'stats_strength') < 96) {
            player.notify("~r~Butina tureti didesni jegos igudi, nei 95%");
            return;
        }

        if (user.get(target, 'online_time') < 170) {
            player.notify("~r~Sis žaidėjas yra naujokas ir jam neleidžiama");
            return;
        }
        ...
        if (random < 50) {
            ...
            chat.sendMeCommand(player, "užsimojo kumščiu ir smogė priešais esančiam vyrui ir tiksliu smūgiu į žandikaulį išmušė iš vėžių.");
            ...
            chat.sendMeCommand(player, "užsimojo kumščiu ir trenkė priešais esančiam vyrui.");
        }
    }
    else
        player.notify('~r~Salia jusu nera ne vieno žmogaus');
...
mp.events.addRemoteCounted('server:user:grabById', (player, targetId) => {
    ...
    if (mp.players.exists(target)) {
        ...
        if (target.health == 0) {
            player.notify("~r~Negalima apiplesti komos istikto žmogaus");
            return;
        }
        ...
        if (user.hasById(user.getId(target), 'sellUser')) {
            player.notify('~r~Neseniai buvo apiplestas žaidejas');
            return;
        }
        ...
        target.notify('~r~Is jusu buvo atimta ~s~' + methods.moneyFormat(money));
        player.notify('~r~Jus atemete ~s~' + methods.moneyFormat(money));
...
mp.events.addRemoteCounted('server:user:getInvById', (player, targetId) => {
    ...
    if (pl && mp.players.exists(pl)) {
        if (pl.health <= 0) {
            player.notify('~r~Negalima ieskoti mirusiuju');
            return;
        }
        if (!user.isTie(pl) && !user.isCuff(pl)) {
            player.notify('~r~Žaidejas turi buti suristas arba surakintas antrankiais.');
            return;
        }
        ...
        if (!user.isFib(player)) {
            if (user.isPolice(pl) && !user.isTie(pl)) {
                player.notify("~r~Policijos pareigunai negali buti ieskomi, kai jiems uždeti antrankiai, tik kai jie suristi.");
                return;
            }
        }
        ...
        if (user.isTie(pl)) {
            if (!user.hasById(user.getId(pl), 'sellUser')) {
                player.notify('~r~Noredami atlikti paieska ekranuose, pirmiausia turite atiduoti žaideja naudodami komanda ecorp -user -getpos, tada susieti ji su ekranais (jei jis nera susietas) ir atlikti paieska.');
                return;
            }
        }
        ...
        if (user.isCuff(pl)) {
            ...
            if (!canFrisk) {
                player.notify('~r~Galima ieskoti tik LSPD / BCSD / FIB stociu');
                return;
            }
        }
...
mp.events.addRemoteCounted('server:user:taskRemoveMaskById', (player, targetId) => {
    ...
    if (nplayer.health <= 1) {
        player.notify('~r~Žaidejas komoje');
        return;
    }
    ...
    player.notify('~b~Nuemete žaidejui kauke');
    ...
    try {
        ...
        if (pl && mp.players.exists(pl)) {
            ...
            if (v && mp.vehicles.exists(v)) {
                ...
                player.notify('~g~Jus itempete žmogu i transporta');
                pl.notify('~r~Jus itempe i transporta');
            } else {
                player.notify('~r~Salia jusu nera transporto');
            }
    ...
    try {
        ...
        if (pl && mp.players.exists(pl)) {
            ...
            if (pl.vehicle && mp.vehicles.exists(pl.vehicle)) {
                ...
                player.notify('~g~Jus ismetete žmogu is transporto');
                pl.notify('~r~Jus ismete is transporto');
            } else {
                player.notify('~r~Žaidejas ne transporte');
            }
        }
        else
            player.notify('~r~Salia jusu nera ne vieno žmogaus');
...
mp.events.addRemoteCounted('server:user:giveMoneyToPlayerId', (player, playerRemoteId, money) => {
    ...
    if (user.getCashMoney(player) < money) {
        player.notify("~r~Jus neturite tiek pinigu");
        return;
    }
    ...
    if (remotePlayer && user.isLogin(remotePlayer)) {
        ...
        remotePlayer.notify('Jums perdave ~g~' + methods.moneyFormat(money));
        player.notify('Jūs perdavėte ~g~' + methods.moneyFormat(money));
...
mp.events.addRemoteCounted('server:user:askDiceToPlayerIdYes', (player, playerRemoteId, sum) => {
    ...
    if (user.isLogin(remotePlayer)) {
        ...
        if (user.getCashMoney(remotePlayer) < sum) {
            remotePlayer.notify('~y~Su savimi neturite pinigu statymui.');
            return;
        }
        if (user.getCashMoney(player) < sum) {
            player.notify('~y~Su savimi neturite pinigu statymui.');
            return;
        }
        ...
        if (generateTarget > generatePlayer) {
            user.addCashMoney(remotePlayer, sum, 'Laimėti kauliukai');
            user.removeCashMoney(player, sum, 'Pralaimėti kauliukai');
            remotePlayer.notify('~g~Jus laimejote ' + methods.moneyFormat(sum));
            player.notify('~y~Jus pralaimejote ' + methods.moneyFormat(sum));
        }
        else if (generateTarget < generatePlayer) {
            user.addCashMoney(player, sum, 'Laimėti kauliukai');
            user.removeCashMoney(remotePlayer, sum, 'Pralaimėti kauliukai');
            player.notify('~g~Jus laimejote ' + methods.moneyFormat(sum));
            remotePlayer.notify('~y~Jus pralaimejote ' + methods.moneyFormat(sum));
        }
        else {
            player.notify('~y~Sužaidete lygiosiomis');
            remotePlayer.notify('~y~Sužaidete lygiosiomis');
        }
    }
});
...
mp.events.addRemoteCounted('server:sendAsk', (player, message) => {
    ...
    if (message === undefined || message === 'undefined')
        return;
    player.outputChatBoxNew(`[${chat.getTime()}] !{#FFC107}Klausimas ${user.getRpName(player)} (${player.id}):!{#FFFFFF} ${message}`);
    mp.players.forEach(function (p) {
        if (!user.isLogin(p))
            return;
        if (user.isHelper(p))
            p.outputChatBoxNew(`[${chat.getTime()}] !{#FFC107}Klausimas iš ${user.getRpName(player)} (${player.id}):!{#FFFFFF} ${message}`);
    });
...
mp.events.addRemoteCounted('server:sendReport', (player, message) => {
    ...
    player.outputChatBoxNew(`[${chat.getTime()}] !{#f44336}Skundas ${user.getRpName(player)} (${player.id}):!{#FFFFFF} ${message}`);
    mp.players.forEach(function (p) {
        if (!user.isLogin(p))
            return;
        if (user.isAdmin(p) && !user.get(p, 'status_rp')) {
            if (user.get(player, 'status_media') > 0)
                p.outputChatBoxNew(`[${chat.getTime()}] !{#3F51B5}[MEDIA] Skundas iš ${user.getRpName(player)} (${player.id}):!{#FFFFFF} ${message}`);
            else
                p.outputChatBoxNew(`[${chat.getTime()}] !{#f44336}Skundas iš ${user.getRpName(player)} (${player.id}):!{#FFFFFF} ${message}`);
        }
    });
...
mp.events.addRemoteCounted('server:sendAnswerAsk', (player, id, msg) => {
    ...
    mp.players.forEach(function (p) {
        if (!user.isLogin(p))
            return;
        if (user.isHelper(p))
            p.outputChatBoxNew(`[${chat.getTime()}] !{#FBC02D}Atsakymas į skundą ${user.getRpName(player)} žaidėjui ${id}:!{#FFFFFF} ${msg}`);
        if (p.id != id)
            return;
        p.outputChatBoxNew(`[${chat.getTime()}] !{#FFC107}Atsakymas į skundą ${user.getRpName(player)}:!{#FFFFFF} ${msg}`);
        ...
        player.notify(`~g~Dekojame, jog padedate serverio žaidejams`);
       // user.addCashMoney(player, money, `Atsakymas į klausimą (${methods.removeQuotes(methods.removeQuotes2(msg)).substring(0, 230)})`)
...
mp.events.addRemoteCounted('server:sendAnswerReport', (player, id, msg) => {
    ...
    mp.players.forEach(function (p) {
        if (!user.isLogin(p))
            return;
        if (user.isAdmin(p))
            p.outputChatBoxNew(`[${chat.getTime()}] !{#d32f2f}Atsakymas iš administratorių ${user.getRpName(player)} žaidėjui ${id}:!{#FFFFFF} ${msg}`);
        if (p.id != id)
            return;
        p.call('client:mainMenu:addReport', [msg, `${user.getRpName(player)} (${player.id})`]);
        p.outputChatBoxNew(`[${chat.getTime()}] !{#f44336}Atsakymas iš administratorių ${user.getRpName(player)}:!{#FFFFFF} ${msg}`);
...
mp.events.addRemoteCounted('server:startSpecMissionLspd', (player, vId) => {
    try {
        if (user.isLogin(player)) {
            ...
            if (canUse >= 3) {
                player.notify('~r~Vienu metu prieinami tik 3 dronai');
                return;
            }
...
mp.events.addRemoteCounted('server:startSpecMissionSmall', (player, vId) => {
    try {
        if (user.isLogin(player)) {
            ...
            if (canUse >= 2) {
                player.notify('~r~Vienu metu prieinami tik 2 dronai');
                return;
            }
...
mp.events.addRemoteCounted('server:admin:gangZone:editPos', (player, id) => {
    ...
    try {
        ...
        player.notify('~b~Atnaujintos koordinates');
    }
    catch (e) {
        methods.debug(e);
    }
...
mp.events.addRemoteCounted('server:admin:gangZone:edit', (player, id, key, val) => {
    ...
    try {
        ...
        player.notify(`~b~Verte ${key} buvo atnaujinta i ${val}`);
    }
    catch (e) {
        methods.debug(e);
    }
...
mp.events.addRemoteCounted('server:ems:removeObject', (player, id) => {
    try {
        ...
        try {
            ...
            user.sendSmsBankOperation(player, `Priemokos įregistravimas ${methods.moneyFormat(200)}`);
        }
        catch (e) {
...
mp.events.addRemoteCounted('server:usmc:vehicleUnload', (player) => {
    try {
        ...
        if (vehicles.exists(v)) {
            ...
            user.addMoney(player, 1000, 'USMC premija');
            coffer.addMoney(coffer.getIdByFraction(4), 500000);

            mp.players.forEach(p => {
                if (user.isUsmc(p)) {
                    user.addMoney(player, 5000, 'USMC premija');
                    player.notify('~g~Už sunkvežimio pristatyma gavote 5000€ premija');
                }
            })
        }
        else {
...
mp.events.addRemoteCounted('server:invader:delNews', (player, id) => {
    ...
    player.notify('~b~Istrynete naujiena #' + id);
});
...
mp.events.addRemoteCounted('server:invader:sendAd', (player, id, title, name, text, phone) => {
    ...
    user.addPayDayMoney(player, 300, 'Redaguotas skelbimas');
    player.notify('~g~Už redaguota skelbima gavote 300€ premija');

    discord.sendAd(title, name, textRemove, methods.phoneFormat(phone), editor, player.socialClub);

    mp.players.forEach(p => {
        user.sendPhoneNotify(p, 'Life Invader', '~g~Reklama | ' + title, text, 'CHAR_LIFEINVADER');
    });
});
...
mp.events.addRemoteCounted('server:invader:delAd', (player, id) => {
    ...
    user.addPayDayMoney(player, 100, 'Redaguotas skelbimas');

    mysql.executeQuery(`DELETE FROM rp_inv_ad WHERE id = ${methods.parseInt(id)}`);
    player.notify('~b~Istrynete skelbima #' + id);
    player.notify('~g~Už redaguota skelbima gavote 100€ premija');
});
...
mp.events.addRemoteCounted('server:invader:sendAdTemp', (player, text) => {
    ...
    if (user.getBankMoney(player) < 500) {
        user.sendSmsBankOperation(player, 'Neužtenka lėšų', 'Nepavykusi operacija');
        return;
    }
    ...
    user.removeBankMoney(player, 500, 'Skelbimo siuntimas');
    coffer.addMoney(8, 200);
    methods.saveFractionLog(
        'Sistema',
        `Reklaminis skelbimas`,
        `Biudžeto papildymas: ${methods.moneyFormat(200)}`,
        7
    );

    mysql.executeQuery(`INSERT INTO rp_inv_ad_temp (name, text, phone) VALUES ('${name}', '${text}', '${phone}')`);

    user.sendPhoneNotify(player, 'Life Invader', '~g~Reklama', 'Jusu skelbimas nagrinejamas', 'CHAR_LIFEINVADER');
});
...
mp.events.addRemoteCounted('server:invader:deleteAdTemp', (player, id) => {
    ...
    user.addPayDayMoney(player, 100, 'Ištrintas skelbimas #' + id);
    mysql.executeQuery(`DELETE FROM rp_inv_ad_temp WHERE id = ${methods.parseInt(id)}`);
});
    ...
    try {
        ...
        if (player.vehicle && player.seat == -1) {
            ...
            if (player.vehicle.getVariable('isStartDuty')) {
                player.notify('~r~Jus jau gavote užduoti');
                return;
            }

            if (player.vehicle.getOccupants().length == 1) {
                player.notify('~b~Galite dirbti tik su partneriais!');
            }
            else {
                let isStart = false;
                    ...
                    try {
                        ...
                        if (user.get(p, 'job') == 10) {
                            ...
                            if (user.getRpName(p) == user.getRpName(player))
                                return;
                            p.notify('~b~Jus gavote užduoti');
                            player.notify('~b~Partneris: ~s~ID: ' + p.id);
        ...
        try {
            ...
            if (vehicles.exists(v)) {
                ...
                if (methods.parseInt(v.getVariable('gr6Money')) <= 1) {
                    player.notify('~r~Masinoje nera lesu');
                    return;
                }
                    ...
                    try {
                        ...
                        if (user.has(p, 'gr6') && user.get(p, 'gr6') === v.id) {
                            ...
                            if (user.get(p, 'vip_type') === 1) {
                                desc = '\n~y~VIP LIGHT 5 % atlyginimo padidinimas';
                                money = money * 1.05;
                            }
                            if (user.get(p, 'vip_type') === 2) {
                                desc = '\n~y~VIP HARD 10 % atlyginimo padidinimas ';
                                money = money * 1.1;
                            }
                            ...
                            if (fId > 0) {
                                ...
                                if (fData.get('level') > 4) {
                                    ...
                                    desc = '\n~y~Seimos 30 % atlyginimo padidinimas';
                                }
                                if (fData.get('level') > 5) {
                                    family.addMoney(fId, currentMoney * 0.3, 'Išėjimas iš inkasatorių darbo');
                                }
                            }

                            user.addMoney(p, currentMoney, 'Inkasatorių darbo užmokestis');
                            coffer.removeMoney(currentMoney + methods.parseFloat(currentMoney / 10));
                            p.notify('~g~Jus uždirbote: ~s~' + methods.moneyFormat(currentMoney) + desc);
                            user.reset(p, 'gr6');
                            user.giveJobSkill(p);
                            ...
                            p.notify('~r~Jus nesate partneris su ID: ' + player.id);
                            p.notify('~r~Darbo užmokescio jus negavote');
                        }
                    }
                    catch (e) {
                        methods.debug(e);
    ...
    try {
        ...
        if (veh && player.seat == 0) {
            if (veh.getVariable('owner_id') == user.getId(player)) {
                    ...
                    setTimeout(function () {
                        try {
                            if (user.isLogin(player)) {
                                ...
                                user.addMoney(player, 4500, 'Inkasatorių darbinės tr. priemonės grąžinimas');
                                player.notify('~b~Grazinote transporto priemone i garaža');
                            }
                        }
                        catch (e) {
                            methods.debug(e);
                ...
                player.notify('~r~Ne jus issinuomavote, ne jums duoti.');
            }
        }
    }
    catch (e) {
        ...
        if (player.vehicle && player.seat == 0) {
            if (player.vehicle.getVariable('job') == 10) {
                ...
                setTimeout(function () {
                    ...
                    setTimeout(function () {
                        user.hideLoadDisplay(player);
                        user.addCryptoMoney(player, money / 1000, 'Apiplėšimas');
                        player.notify('~b~Jus apiplesete transporta už: ~s~' + methods.cryptoFormat(money / 1000));
                    }, 500);
                }, 700);
            }
            else {
                player.notify('~r~Tai ne inkasatoriu masina');
            }
        }
    }
    catch (e) {
...
mp.events.addRemoteCounted('server:vehicles:addNew', (player, model, count) => {
    if (user.isAdmin(player)) {
        ...
        player.notify('~g~Pridetas transportas i automobiliu rinka. Kiekis: ~s~' + count)
    }
});
...
mp.events.addRemoteCounted('server:phone:call', (player, phone) => {
    ...
    try {
        ...
        if (player.getVariable('callId')) {
            player.notify('~r~Jus jau kalbate');
            return;
        }
        
        let isFind = false;
        phone = methods.parseInt(phone);
        let text = '~r~Abonentas nepasiekiamas';
        mp.players.forEach(p => {
            if (user.isLogin(p) && methods.parseInt(user.get(p, 'phone')) === phone) {

                if (p.callWait || p.getVariable('callId')) {
                    text = '~r~Abonentas jau kalbasi telefonu, perskambinkite vėliau';
                    return;
                }

                isFind = true;
                player.notify('~b~Palaukite, kol abonentas priims skambuti');
                p.notify('~b~Gavote ieinanti skambuti');
    ...
    try {
        if (player.callWait >= 0) {
            ...
            if (user.isLogin(target)) {
                ...
                player.notify('~r~Skambutis buvo nutrauktas, abonentas nera pasiekiamas')
            }
        }
    }
    catch (e) {
...
mp.events.addRemoteCounted('server:phone:editContact', (player, json) => {
    ...
    try {
        ...
        player.notify('~y~Kontaktas redaguotas');
    }
    catch (e) {
        methods.debug(e);
    }
...
mp.events.addRemoteCounted('server:phone:favoriteContact', (player, json) => {
    ...
    try {
        ...
        player.notify('~y~Kontaktas pridetas prie megstamiausiu');
    }
    catch (e) {
        methods.debug(e);
    }
...
mp.events.addRemoteCounted('server:phone:deleteContact', (player, json) => {
    ...
    try {
        ...
        player.notify('~y~Kontaktas istrintas');
    }
    catch (e) {
        methods.debug(e);
    }
...
mp.events.addRemoteCounted('server:phone:addContact', (player, json) => {
    ...
    try {
        ...
        if (contact.name === '') {
            player.notify('~y~Laukas "Vardas" yra tuscias');
            return;
        }
        ...
        player.notify('~g~Pridetas kontaktas');
        setTimeout(function () {
    ...
    try {
        ...
        mp.players.forEach(p => {
            if (user.isLogin(p) && user.get(p, 'phone_type') > 0 && user.get(p, 'phone') === methods.parseInt(phoneNumber)) {
                user.sendPhoneNotify(p, methods.phoneFormat(user.get(player, 'phone')), '~b~Nauja zinute', message);
...
mp.events.addRemoteCounted('server:user:askAnim', (player, id, animId) => {
    ...
    if (user.isLogin(target)) {
        if (target.id == player.id) {
            player.notify('~r~Klaida');
            return;
        }

        if (methods.distanceToPos(target.position, player.position) > 5) {
            player.notify('~r~Jus esate per toli vienas nuo kito');
            return;
        }
        let desc = [
            'Žaidėjas nori susipažinti',
            'Žaidėjas nori susipažinti',
            'Žaidėjas nori duoti penkis',
            'Žaidėjas nori pasibučiuoti',
            'Žaidėjas nori blowjobo',
            'Žaidėjas nori sekso',
        ];
        target.call('client:menuList:showAskAnimMenu', [player.id, desc[animId], animId])
    }
    else {
        player.notify('~r~Žaidejas nerastas');
    }
});
...
mp.events.addRemoteCounted('server:user:invite', (player, id) => {
    ...
    if (user.isLogin(target)) {
        if (target.id == player.id) {
            player.notify('~r~Sveiki, ketinau cia iterpti anekdota, bet nesugalvojau, bet kaip jus ketinate priimti save i organizacija, kurios nariu jau esate?');
            return;
        }
        ...
        if (user.get(target, 'fraction_id') > 0 || user.get(target, 'fraction_id2') > 0) {
            player.notify('~r~Žaidejas jau priklauso organizacijai');
            return;
        }
...
mp.events.addRemoteCounted('server:user:invite:accept', (player, id) => {
    ...
    if (user.isLogin(inviter)) {
        if (methods.distanceToPos(player.position, inviter.position) > 5) {
            inviter.notify('~r~Jus esate per toli vienas nuo kito');
            return;
        }

        user.addHistory(player, 0, 'Buvo priimtas į organizaciją ' + user.getFractionName(inviter) + '. Priėmė: ' + user.getRpName(inviter));
        ...
        player.notify('~g~Jus prieme i organizacija ' + user.getFractionName(inviter));
        inviter.notify('~b~Jus pasamdete darbuotoja: ~s~' + user.getRpName(player));
...
mp.events.addRemoteCounted('server:user:invite2:accept', (player, id) => {
    ...
    if (user.isLogin(inviter)) {
        ...
        player.notify('~g~Jus prieme i organizacija');
        inviter.notify('~b~Jus priemete: ~s~' + user.getRpName(player));
...
mp.events.addRemoteCounted('server:phone:inviteFamily', (player, id) => {
    ...
    if (user.isLogin(target)) {
        ...
        if (user.get(target, 'family_id') > 0) {
            player.notify('~r~Žaidejas jau priklauso seimai');
            return;
        }
...
mp.events.addRemoteCounted('server:user:invitef:accept', (player, id) => {
    ...
    if (user.isLogin(inviter)) {
        ...
        player.notify('~g~Jus prieme i seima');
        inviter.notify('~b~Jus priemete: ~s~' + user.getRpName(player));
...
mp.events.addRemoteCounted('server:phone:mafiaClearWanted', (player, id) => {
    ...
    if (user.isLogin(target)) {
        ...
        if (user.get(target, 'wanted_level') === 0) {
            player.notify('~r~Žaidejas neturi ieskomumo');
            return;
        }
        ...
        if (user.getCashMoney(target) < price) {
            player.notify('~r~Žaidejui nepakanka lesu');
            return;
        }
...
mp.events.addRemoteCounted('server:phone:mafiaGiveShop', (player, id) => {
    ...
    if (user.isLogin(target)) {
        ...
        player.notify('~g~Jus davete arbatpinigiu');
    }
    else {
        player.notify('~r~Žaidejas nerastas');
    }
...
mp.events.addRemoteCounted('server:user:clearByMafia', (player, id, price) => {
    ...
    if (user.isLogin(target)) {
        ...
        if (user.get(player, 'wanted_level') === 0) {
            player.notify('~r~Jus neturite ieskomumo');
            return;
        }
        ...
        target.notify(`~g~Ivykdete paieska ir užsidirbote ${methods.moneyFormat(price / 2)}`);

        fraction.addMoney(user.get(target, 'fraction_id2'), (price / 2 / 1000), `Pelnas iš paieškos panaikinimo (${user.getRpName(target)})`);
        user.addMoney(target, price / 2, 'Pajamos, gautos atlikus paiešką');
        user.removeMoney(player, price, 'Paieškos išvalymas');

        user.set(player, 'wanted_level', 0);
        user.set(player, 'wanted_reason', '');
        player.notifyWithPicture('Pranesimas', 'Police Department', 'Jus nebesate ieskomas asmuo', 'WEB_LOSSANTOSPOLICEDEPT', 2);
        user.addHistory(player, 1, 'Atlikta paieška');
    }
    else {
        player.notify('~r~Žaidejas nerastas');
    }
    ...
    mp.vehicles.forEach(v => {
        if (v.getVariable('vid') == id) {
            if (v.getOccupants().length > 0) {
                player.notify('~r~Pavogtos transporto priemones');
                return;
            }

            if (user.getBankMoney(player) < price) {
                player.notify(`~r~Butina tureti ${methods.moneyFormat(price)} banko korteleje už naudojimasi avarines tarnybos paslaugomis`);
                return;
            }

            user.removeBankMoney(player, price, 'Vilkikų paslaugos');
            coffer.addMoney(1, price);

            vehicles.respawn(v);
            player.notify('~g~Jusu transporto priemone netrukus atsidurs stovejimo vietoje');
        }
    });
});
    ...
    mp.vehicles.forEach(v => {
        if (v.getVariable('vid') == id) {
            ...
            else if (v.dimension >= 100000)
            {
                ...
                player.notify('~y~Jusu transporto priemone stovi konfiskuotoje aiksteleje.');
            }
            else
                player.notify('~y~Jusu transporto priemone stovi garaže');
        }
    });
});
    ...
    try {
        ...
        setTimeout(function () {
            try {
                ...
                user.putInVehicle(player, veh, 0);
                player.notify('~y~Nepamirskite sumoketi baudos 2 - Transportas');
...
mp.events.addRemoteCounted('server:user:sellFish', (player, shopId) => {
    ...
    mysql.executeQuery(sql, function (err, rows, fields) {
        ...
        business.addMoney(shopId, price / 10, 'Žuvies pardavimo dalis');
        user.addMoney(player, price, 'Visų žuvų pardavimas');
        inventory.deleteItemsRange(player, 487, 536);
        player.notify(`~b~Jus pardavete žuvi už: ~s~${methods.moneyFormat(price)}`);
        ...
        if (fId > 0) {
            ...
            if (fData.get('level') === 5) {
                if (fData.get('exp') > 100000) {
                    family.addMoney(fId, 7500000, '6 lygio pasiekimų apdovanojimas');
...
mp.events.addRemoteCounted('server:tradeMarket:unrentBeach', (player, idx) => {
    ...
    user.updateClientCache(player);
    player.notify('~y~Nutraukete nuoma');
...
mp.events.addRemoteCounted('server:tradeMarket:rentBlack', (player, idx) => {
    ...
    if (user.getCashMoney(player) < 2000) {
        player.notify('~r~Jus prie saves 2000€ neturite');
        return;
    }
    if (tradeMarket.getBeach(idx, 'rent')) {
        player.notify('~r~Palapine jau yra nuomojama');
...
mp.events.addRemoteCounted('server:tradeMarket:buy', (player, id, price, name, ownerId, payType) => {
    ...
    if (user.getMoney(player, payType) < price) {
        player.notify('~r~Jus prie saves neturite ' + methods.moneyFormat(price));
        return;
    }
    let seller = user.getPlayerById(ownerId);
    if (!user.isLogin(seller))
    {
        player.notify('~r~Pardavejas neprisijunges');
        return;
    }
    ...
    user.removeCashMoney(player, price, 'Pirkimas rinkoje: ' + name, payType);
    user.addCashMoney(seller, priceWithTax, 'Pardavimas rinkoje: ' + name);

    seller.notify(`~g~Jus pardavete ~s~"${name}" ~g~už ~s~${methods.moneyFormat(priceWithTax)}~g~ iskaiciuotas mokestis ~s~${coffer.getTaxIntermediate()}%`);
    player.notify(`~g~Jus nusipirkote ~s~"${name}"~g~ už ~s~${methods.moneyFormat(price)}`);
});
...
mp.events.addRemoteCounted("server:showVehMenu", (player) => {
    ...
    if (player.vehicle && player.seat == 0)
        ...
        player.notify('~r~Jus turite buti transporte');
});
...
mp.events.addRemoteCounted('server:fight:show:accept', (player, playerid, money) => {
    let fighting = mp.players.at(playerid);
    if (!fighting) return;
    fighting.call('client:fight:showacceptmenu', [player.id, money]);
});
mp.events.addRemoteCounted('server:user:acceptedFight', (player, target, money) => {
    let fighting = mp.players.at(target);
    if (!fighting) return;
    user.startFight(player, target, money);
});
        ...
        if (vehicles.exists(vehicle)) {
            ...
            if (vehicle.getVariable('fraction_id')) {
                if (
                    ...
                    player.notify('~r~Jus neturite transporto raktu');
            }
        ...
        stocks.pcList.forEach(function(item) {
            ...
            if (methods.distanceToPos(player.position, new mp.Vector3(x, y, z)) < 1.5) {
                ...
                if (houseData.get('user_id') == user.getId(player))
                    ...
                    player.notify('~r~Jus nesate sandelio savininkas');
            }
        });
    }
...
mp.events.addRemoteCounted('server:user:askAve', (player, id) => {
    ...
    try {
        ...
        if (user.getCashMoney(player) < 10000) {
            player.notify('~r~Jus prie saves 10000€ neturite');
            return;
        }
        if (user.get(player, 'partner') !== '') {
            player.notify('~r~Jus jau susituokete');
            return;
        }
        if (user.get(target, 'partner') !== '') {
            player.notify('~r~Žaidejas jau susituoke');
            return;
        }
        if (player.id === target.id) {
            player.notify('~r~Negalima susituokti paciam su savimi');
            return;
        }
...
mp.events.addRemoteCounted('server:user:askNoAve', (player, id) => {
    ...
    try {
        ...
        if (user.getCashMoney(player) < 1000) {
            player.notify('~r~Jus prie saves 1000€ neturite');
            return;
        }
        if (user.getRpName(player) !== user.get(target, 'partner')) {
            player.notify('~r~Jus nesate susituokes su siuo žmogumi');
            return;
        }
...
mp.events.addRemoteCounted('server:ave:accept', (player, id) => {
    ...
    try {
        ...
        if (user.get(player, 'partner') !== '') {
            player.notify('~r~Jus jau susituokes');
            return;
        }
        if (user.get(target, 'partner') !== '') {
            player.notify('~r~Žaidejas jau susituokes');
            return;
        }
        ...
        user.save(player);
        user.save(target);
        chat.sendToAll('Kunigas', `Sveikiname! Pora suformavo nauja visuomenes vieneta ${user.getRpName(target)} ir ${user.getRpName(player)}`,  chat.clGreen);
        ...
        mysql.executeQuery(`SELECT * FROM users WHERE name = '${name} ${surname}'`, function (err, rows, fields) {
            if (rows.length === 0) {
                try {
                    ...
                    player.notify(`~r~Pavarde pakeista i ${surname}`);
                    user.set(player, 'partner', user.getRpName(target));
                }
                catch (e) {}
            }
            else {
                target.notify('~r~Vardo ir pavardes derinys jau užimtas, todel jis nebuvo pakeistas.')
            }
        });
    }
    catch (e) {
...
mp.events.addRemoteCounted('server:noave:accept', (player, id) => {
    ...
    try {
        ...
        player.notify('~r~Jus nutraukete santuoka');
        target.notify('~r~Jus nutraukete santuoka');
...
mp.events.addRemoteCounted('server:mechanic:fuel', (player, id, count, price) => {
    ...
    if (!target.vehicle) {
        player.notify('~r~Žaidejas turi buti transporte');
        return;
    }
...
mp.events.addRemoteCounted('server:mechanic:fuel:accept', (player, id, count, price) => {
    try {
        ...
        if (!player.vehicle) {
            player.notify('~r~Jus ne transporte');
            return;
        }
        if (!target.vehicle) {
            player.notify('~r~Mechanikas ne transporte');
            return;
        }
        user.removeMoney(player, price, 'Mechaniko paslaugos');
        user.addMoney(target, price, 'Mechaniko paslaugos');

        let vInfo = methods.getVehicleInfo(player.vehicle.model);
        if (vInfo.fuel_type === 0) {
            player.notify(`~r~Si transporto rusis nenaudoja degalu`);
            return;
        }

        if (vInfo.fuel_full <= vehicles.getFuel(player.vehicle) + count) {
            player.notify('~r~Transporto kuras jau yra pildomas');
            return;
        }
        ...
        player.notify(`~g~Jums pripyle ~s~${count} vnt.~g~ degalu už ~s~${methods.moneyFormat(price)}`);
        target.notify(`~g~Jus i transporta pripylete ~s~${count} vnt.~g~ degalu už ~s~${methods.moneyFormat(price)}`);
...
mp.events.addRemoteCounted('server:mechanic:fix:accept', (player, id, price) => {
    try {
        ...
        if (player.vehicle.dead) {
            player.notify('~r~Transportas susprogdintas');
            return;
        }
        user.removeMoney(player, price, 'Mechaniko paslaugos');
        user.addMoney(target, price, 'Mechaniko paslaugos');

        player.notify(`~g~Jusu transporto priemone buvo suremontuota už ~s~${methods.moneyFormat(price)}`);
        target.notify(`~g~Jus suremontavote transporto priemone už ~s~${methods.moneyFormat(price)}`);
...
mp.events.addRemoteCounted('server:mechanic:flip:accept', (player, id, price) => {
    try {
        ...
        player.notify(`~g~Jums apverte transporta už ~s~${methods.moneyFormat(price)}`);
        target.notify(`~g~Jus apvertete transporta už ~s~${methods.moneyFormat(price)}`);
...
mp.events.addRemoteCounted('server:houses:addUser', (player, buyerId) => {
    ...
    if (user.get(player, 'house_id') === 0) {
        player.notify('~r~Jus neturite namo');
        return;
    }

    if (user.getCashMoney(player) < 10000) {
        player.notify('~r~Jus neturite pakankamai lesu rankose');
        return;
    }

    let hInfo = houses.getHouseData(user.get(player, 'house_id'));
    if (hInfo.get('user_id') != user.get(player, 'id')) {
        player.notify('~r~Sis namas jums nepriklauso');
        return;
    }
    ...
    if (user.isLogin(buyer)) {
        ...
        if (user.get(buyer, 'house_id') > 0) {
            player.notify('~r~Žaidejas jau turi nama');
            buyer.notify('~r~Jus jau turite nama');
            return;
        }
        ...
        houses.getCountLiveUser(user.get(player, 'house_id'), function (count) {
            ...
            if (count == hInfo.get('max_roommate')) {
                player.notify('~r~Namuose nebera vietos\nTurite tureti didesnius namus, kad galetumete tureti daugiau draugu.');
                return;
            }
            ...
            player.notify('~b~Jus perkelete žaideja');
            buyer.notify('~b~Buvote perkeltas i nama');
...
mp.events.addRemoteCounted('server:houses:lawyer:tryaddUser', (player, ownerId, buyerId) => {
    ...
    if (user.isLogin(owner) && user.isLogin(player)) {
        if (user.get(owner, 'house_id') === 0) {
            owner.notify('~r~Jus neturite namo');
            player.notify('~r~Žaidejas neturi namo');
            return;
        }

        if (user.getCashMoney(owner) < 10000) {
            owner.notify('~r~Jums nepakanka lesu rankose');
            player.notify('~r~Žaidejui nepakanka lesu rankose');
            return;
        }
...
mp.events.addRemoteCounted('server:houses:lawyer:addUser', (player, lawyerId, buyerId) => {
    ...
    if (user.isLogin(buyer) && user.isLogin(owner) && user.isLogin(lawyer)) {
        ...
        if (hInfo.get('user_id') != user.get(owner, 'id')) {
            owner.notify('~r~Sis namas jums nepriklauso');
            return;
        }

        if (methods.distanceToPos(buyer.position, owner.position) > 2) {
            owner.notify('~r~Jus esate per toli');
            return;
        }

        if (user.get(buyer, 'house_id') > 0) {
            owner.notify('~r~Žaidejas jau turi nama');
            buyer.notify('~r~Jus jau turite nama');
            return;
        }
        ...
        houses.getCountLiveUser(user.get(owner, 'house_id'), function (count) {
            ...
            --count;
            if (count == hInfo.get('max_roommate')) {
                owner.notify('~r~Namuose nebera vietos\nTurite tureti didesnius namus, kad galetumete tureti daugiau draugu.');
                return;
            }
            ...
            owner.notify('~b~Jus perkelete žaideja');
            buyer.notify('~b~Jus perkele i nama');
            lawyer.notify('~b~Atlikote sandori ir uždirbote ~g~8.000€');
...
mp.events.addRemoteCounted('server:houses:removeMe', (player) => {
    if (!user.isLogin(player))
        return;
    if (user.get(player, 'house_id') === 0) {
        player.notify('~r~Jus niekur negyvenate');
        return;
    }

    if (user.getCashMoney(player) < 1000) {
        player.notify('~r~Jums nepakanka lesu rankose');
        return;
    }

    let hInfo = houses.getHouseData(user.get(player, 'house_id'));
    if (hInfo.get('user_id') == user.get(player, 'id')) {
        player.notify('~r~Tai jusu namas, ji galima tik parduoti');
        return;
    }
    ...
    player.notify('~r~Buvote iskeldintas is namu');
    user.save(player);
});
...
mp.events.addRemoteCounted('server:house:removeId', (player, userId) => {
    ...
    player.notify('~r~Buvote iskeldintas is namo ' + userId);

    let seller = user.getPlayerById(userId);
    if (user.isLogin(seller)) {
        user.set(seller, 'house_id', 0);
        seller.notify('~r~Namo savininkas jus iskeldino is namo');
        user.save(seller);
    }
    else {
...
mp.events.addRemoteCounted('server:houses:sellToPlayer', (player, buyerId, sum) => {
    ...
    if (user.isLogin(buyer)) {
        ...
        buyer.notify('~b~Jums buvo pasiulyta pirkti nama už ~s~' + methods.moneyFormat(sum));
        player.notify('~b~Žaidejui pasiulete pirkti nama');
    }
});
...
mp.events.addRemoteCounted('server:houses:sellToPlayer:accept', (player, houseId, sum, sellerId) => {
    if (!user.isLogin(player))
        return;
    if (user.get(player, 'house_id') > 0) {
        player.notify('~r~Jus jau turite nama');
        return;
    }

    if (user.getCashMoney(player) < sum) {
        player.notify('~r~Jums neužtenka pinigu');
        return;
    }
    ...
    if (user.isLogin(seller)) {
        ...
        if (hId === 0) {
            player.notify('~r~Žaidejas neturi namo');
            seller.notify('~r~Jus neturite namo');
            return;
        }
        ...
        seller.notify('~b~Jus pardavete nama už ~s~' + methods.moneyFormat(sum));
        player.notify('~b~Jus nusipirkote nama už ~s~' + methods.moneyFormat(sum));
        ...
        user.addHistory(seller, 3, 'Pardavė namą ' + hInfo.get('address') + ' №' + hInfo.get('number') + '. Kaina: ' + methods.moneyFormat(sum));
        user.addHistory(player, 3, 'Nusipirko namą ' + hInfo.get('address') + ' №' + hInfo.get('number') + '. Kaina: ' + methods.moneyFormat(sum));
    }
});
...
mp.events.addRemoteCounted('server:condo:sellToPlayer', (player, buyerId, sum) => {
    ...
    if (user.get(player, 'condo_id') === 0) {
        player.notify('~r~Jus neturite buto');
        return;
    }
    ...
    if (user.isLogin(buyer)) {
        ...
        if (user.get(buyer, 'condo_id') > 0) {
            player.notify('~r~Žaidejas jau turi buta');
            buyer.notify('~r~Jus jau turite buta');
            return;
        }
        ...
        buyer.notify('~b~Jums pasiule pirkti buta už ~s~' + methods.moneyFormat(sum));
        player.notify('~b~Jus pasiulete žaidejui pirkti buta');
    }
});
...
mp.events.addRemoteCounted('server:condo:sellToPlayer:accept', (player, houseId, sum, sellerId) => {
    if (!user.isLogin(player))
        return;
    if (user.get(player, 'condo_id') > 0) {
        player.notify('~r~Jus jau turite buta');
        return;
    }
    ...
    if (user.isLogin(seller)) {
        ...
        if (hId === 0) {
            player.notify('~r~Žaidejas neturi buto');
            seller.notify('~r~Jus neturite buto');
            return;
        }
        ...
        seller.notify('~b~Jus pardavete buta už ~s~' + methods.moneyFormat(sum));
        player.notify('~b~Jus nusipirkote buta už ~s~' + methods.moneyFormat(sum));
        ...
        user.addHistory(seller, 3, 'Pardavė butą ' + hInfo.get('address') + ' №' + hInfo.get('number') + '. Kaina: ' + methods.moneyFormat(sum));
        user.addHistory(player, 3, 'Nusipirko butą ' + hInfo.get('address') + ' №' + hInfo.get('number') + '. Kaina: ' + methods.moneyFormat(sum));
    }
});
...
mp.events.addRemoteCounted('server:yacht:sellToPlayer', (player, buyerId, sum) => {
    ...
    if (user.get(player, 'yacht_id') === 0) {
        player.notify('~r~Jus neturite jachtos');
        return;
    }
    ...
    if (user.isLogin(buyer)) {
        ...
        if (user.get(buyer, 'yacht_id') > 0) {
            player.notify('~r~Žaidejas turi jachta');
            buyer.notify('~r~Jus turite jachta');
            return;
        }
        ...
        buyer.notify('~b~Jums pasiule pirkti jachta už ~s~' + methods.moneyFormat(sum));
        player.notify('~b~Jus pasiulete pirkti jachta žaidejui');
    }
});
...
mp.events.addRemoteCounted('server:yacht:sellToPlayer:accept', (player, houseId, sum, sellerId) => {
    if (!user.isLogin(player))
        return;
    if (user.get(player, 'yacht_id') > 0) {
        player.notify('~r~Jus turite jachta');
        return;
    }
    ...
    if (user.isLogin(seller)) {
        ...
        if (hId === 0) {
            player.notify('~r~Žaidejas neturi jachtos');
            seller.notify('~r~Jus neturite jachtos');
            return;
        }
        ...
        seller.notify('~b~Jus pardavete jachta už ~s~' + methods.moneyFormat(sum));
        player.notify('~b~Jus nusipirkote jachta už ~s~' + methods.moneyFormat(sum));
        ...
        user.addHistory(seller, 3, 'Pardavė jachtą ' + hInfo.get('name') + ' №' + hInfo.get('id') + '. Kaina: ' + methods.moneyFormat(sum));
        user.addHistory(player, 3, 'Nusipirko jachtą ' + hInfo.get('name') + ' №' + hInfo.get('id') + '. Kaina: ' + methods.moneyFormat(sum));
    }
});
...
mp.events.addRemoteCounted('server:business:sellToPlayer', (player, buyerId, sum) => {
    ...
    if (user.get(player, 'business_id') === 0) {
        player.notify('~r~Jus neturite verslo');
        return;
    }
    ...
    if (user.isLogin(buyer)) {
        ...
        if (user.get(buyer, 'business_id') > 0) {
            player.notify('~r~Žaidejas turi versla');
            buyer.notify('~r~Jus turite versla');
            return;
        }

        if (user.get(buyer, 'age') < 21) {
            player.notify('~r~Žaidejas turi buti 21 metu');
            return false;
        }
        ...
        buyer.notify('~b~Jums buvo pasiulyta pirkti versla už ~s~' + methods.moneyFormat(sum));
        player.notify('~b~Jus pasiulete pirkti versla žaidejui');
    }
});
...
mp.events.addRemoteCounted('server:business:sellToPlayer:accept', (player, houseId, sum, sellerId) => {
    ...
    if (user.get(player, 'business_id') > 0) {
        player.notify('~r~Jus turite versla');
        return;
    }
    ...
    if (user.get(player, 'reg_status') < 2) {
        player.notify('~r~Butina tureti pilietybe');
        return false;
    }

    if (!user.get(player, 'biz_lic')) {
        player.notify('~r~Neturite verslo licencijos\nGalite ja isigyti is valstybes pareigunu.');
        return false;
    }

    if (user.get(player, 'fraction_id') == 1) {
        player.notify('~r~Valstybes tarnautojams draudžiama pirkti imones');
        return false;
    }
    ...
    if (user.isLogin(seller)) {
        ...
        if (hId === 0) {
            player.notify('~r~Žaidejas neturi verslo');
            seller.notify('~r~Jus neturite verslo');
            return;
        }
        ...
        seller.notify('~b~Jus pardavete versla už ~s~' + methods.moneyFormat(sum));
        player.notify('~b~Jus nusipirkote versla už ~s~' + methods.moneyFormat(sum));
        ...
        user.addHistory(seller, 3, 'Pardavė verslą ' + hInfo.get('name') + '. Kaina: ' + methods.moneyFormat(sum));
        user.addHistory(player, 3, 'Nusipirko verslą ' + hInfo.get('name') + '. Kaina: ' + methods.moneyFormat(sum));
    }
});
...
mp.events.addRemoteCounted('server:apartments:sellToPlayer:accept', (player, houseId, sum, sellerId) => {
    ...
    if (user.getCashMoney(player) < sum) {
        player.notify('~r~Jums neužtenka pinigų');
        return;
    }
...
mp.events.addRemoteCounted('server:stock:sellToPlayer', (player, buyerId, sum) => {
    ...
    if (user.get(player, 'stock_id') === 0) {
        player.notify('~r~Jus neturite sandelio');
        return;
    }
    ...
    if (user.isLogin(buyer)) {
        ...
        if (user.get(buyer, 'stock_id') > 0) {
            player.notify('~r~Žaidejas turi sandeli');
            buyer.notify('~r~Jus turite sandeli');
            return;
        }
        ...
        buyer.notify('~b~Jums pasiule pirkti sandeli už ~s~' + methods.moneyFormat(sum));
        player.notify('~b~Jus pasiulete pirkti sandeli žaidejui');
    }
});
...
mp.events.addRemoteCounted('server:stock:sellToPlayer:accept', (player, houseId, sum, sellerId) => {
    if (!user.isLogin(player))
        return;
    if (user.get(player, 'stock_id') > 0) {
        player.notify('~r~Jus turite sandeli');
        return;
    }
    ...
    if (user.isLogin(seller)) {
        ...
        if (hId === 0) {
            player.notify('~r~Žaidejas neturi sandelio');
            seller.notify('~r~Jus neturite sandelio');
            return;
        }
        ...
        seller.notify('~b~Jus pardavete sandeli už ~s~' + methods.moneyFormat(sum));
        player.notify('~b~Jus nusipirkote sandeli už ~s~' + methods.moneyFormat(sum));
        ...
        user.addHistory(seller, 3, 'Pardavė sandėlį ' + hInfo.get('address') + ' №' + hInfo.get('number') + '. Kaina: ' + methods.moneyFormat(sum));
        user.addHistory(player, 3, 'Nusipirko sandėlį ' + hInfo.get('address') + ' №' + hInfo.get('number') + '. Kaina: ' + methods.moneyFormat(sum));
    }
});
...
mp.events.addRemoteCounted('server:car:takeOffsetMoney', (player, slot) => {
    ...
    if (user.get(player, 'car_id' + slot) == 0) {
        player.notify('~r~Jus neturite transporto');
        return;
    }
    ...
    user.addMoney(player, offset, 'Kompensacija už transporto žalą');
    vehicles.updatePrice(vData.get('id'), vInfo.price);
    player.notify('~y~Jums buvo kompensuota pasikeitusi vežimo kaina, sudaranti ' + methods.moneyFormat(offset));
});
...
mp.events.addRemoteCounted('server:car:giveOffsetMoney', (player, slot) => {
    ...
    if (user.getMoney(player) < offset) {
        player.notify(`~r~Jus prie saves ${methods.moneyFormat(offset)}`);
        return;
    }
    user.removeMoney(player, offset, 'Kompensacija už transporto žalą');
    vehicles.updatePrice(vData.get('id'), vInfo.price);
    player.notify('~y~Jums buvo kompensuota pasikeitusi transporto kaina, sudaranti ' + methods.moneyFormat(offset) + '.\nDabar galite parduoti transporto priemones is ranku i rankas');
});
...
mp.events.addRemoteCounted('server:car:sellToBu', (player, price, slot) => {
    ...
    if (freePos[0] === 0) {
        player.notify('~y~Automobiliu turguje laisvu vietu nera');
        user.addCashMoney(player, 1000, 'Grąžinamosios išmokos iš naudotų automobilių rinkos');
        return;
    }
    ...
    vehicles.save(vId);
    player.notify('~y~Jus idejote tr. priemone i pardavima');
});
...
mp.events.addRemoteCounted('server:car:sellFromBu', (player, slot) => {
    ...
    mp.vehicles.forEach(v => {
        if (v.getVariable('vid') == vId) {
            ...
            vehicles.save(vId);
            vehicles.respawn(v);
            player.notify('~y~Jus isemete transporto priemone is rinkos\n~g~Jusu transporto priemone netrukus atsidurs stovejimo aiksteleje');
        }
    });
});
...
mp.events.addRemoteCounted('server:car:sellToPlayer', (player, buyerId, sum, slot) => {
    ...
    if (user.isLogin(buyer)) {
        ...
        if (isValid) {
            ...
            buyer.notify('~b~Jums pasiule pirkti ' + vInfo.get('name') + ' už ~s~' + methods.moneyFormat(sum));
            player.notify('~b~Jus pasiulete pirkti ' + vInfo.get('name') + ' žaidejui');
        }
        else {
            buyer.notify('~r~Neturite laisvu slotu');
            player.notify('~r~Žaidejas neturi laisvu tr. priemones slotu');
        }
    }
});
...
mp.events.addRemoteCounted('server:car:sellToPlayer:accept', (player, houseId, sum, sellerId, slot) => {
    ...
    if (slotBuy === 0) {
        player.notify('~r~Jus neturite laisvu slotu');
        return;
    }
    ...
    if (user.isLogin(seller)) {
        ...
        if (hId === 0) {
            player.notify('~r~Žaidejas neturi transporto');
            seller.notify('~r~Jus neturite transporto');
            return;
        }
        ...
        seller.notify('~b~Jus pardavete tr. priemone už ~s~' + methods.moneyFormat(sum));
        player.notify('~b~Jus nusipirkote tr. priemone už ~s~' + methods.moneyFormat(sum));
        ...
        user.addHistory(seller, 3, 'Pardavė transportą ' + vInfo.get('name') + ' | ' + vInfo.get('number') + '. Kaina: ' + methods.moneyFormat(sum));
        user.addHistory(player, 3, 'Nusipirko transportą ' + vInfo.get('name') + ' | ' + vInfo.get('number') + '. Kaina: ' + methods.moneyFormat(sum));
    }
});
...
mp.events.addRemoteCounted('server:car:buyBot', (player, remoteId, vid, payType) => {
    ...
    if (sum === 0) {
        player.notify('~r~Transporto priemones pasalintos is pardavimo');
        return;
    }
    ...
    if (user.isLogin(seller)) {
        ...
        if (slot === 0) {
            player.notify('~r~Transporto pirkimo klaida');
            return;
        }
        ...
        user.removeMoney(player, sum, `Transporto pirkimas ${vData.get('name')}`, payType);

        seller.notify(`~b~Jus pardavete ${vData.get('name')} už ~s~` + methods.moneyFormat(sum * 0.99));
        player.notify('~b~Jus nusipirkote tr. priemone už ~s~' + methods.moneyFormat(sum));
        ...
        user.addHistory(seller, 3, 'Pardavė transportą ' + vData.get('name') + ' | ' + vData.get('number') + '. Kaina: ' + methods.moneyFormat(sum));
        user.addHistory(player, 3, 'Nusipirko transportą ' + vData.get('name') + ' | ' + vData.get('number') + '. Kaina: ' + methods.moneyFormat(sum));
    }
    else {
        player.notify('~r~Žaidejas nera prisijunges');
    }
});
...
mp.events.addRemoteCounted('server:fraction:vehicleNewRank', (player, id, rank) => {
    ...
    player.notify('~b~Pasikeite galimybe naudotis transportu');
});
...
mp.events.addRemoteCounted('server:fraction:vehicleNewDep', (player, id, dep) => {
    ...
    player.notify('~b~Perkelete transporta i kita skyriu');
});
...
mp.events.addRemoteCounted('server:fraction:vehicleRespawn', (player, id) => {
    ...
    player.notify('~b~Buvo pastebetos transporto priemones');
});
...
mp.events.addRemoteCounted('server:fraction:getDrugVans', (player) => {
    ...
    if (currentMed < 500) {
        player.notify('~r~Sandelyje neužtenka medikamentu');
        return;
    }
    ...
    player.notify('~g~Nustatoma transporto priemones žyma');
});
...
mp.events.addRemoteCounted('server:fraction:vehicleBuy', (player, id, price) => {
    ...
    if (coffer.getMoney(cofferId) < price) {
        player.notify('~r~Organizacijos biudžete neužtenka lesu');
        return;
    }
    ...
    mysql.executeQuery(`SELECT * FROM cars_fraction WHERE id = '${id}'`, function (err, rows, fields) {
        rows.forEach(function (item) {
            ...
            methods.saveFractionLog(
                ...
                `Nusipirko transportą ${item['name']}`,
                `Išleista iš biudžeto: ${methods.moneyFormat(price)}`,
                fractionId
            );

            player.notify(`~b~Jus nusipirkote ${item['name']} organizacijai už ${methods.moneyFormat(item['price'])}`);
        });
    });
...
mp.events.addRemoteCounted('server:fraction:vehicleSell', (player, id, price) => {
    ...
    mysql.executeQuery(`SELECT * FROM cars_fraction WHERE id = '${id}'`, function (err, rows, fields) {
        rows.forEach(function (item) {
            methods.saveFractionLog(
                user.getRpName(player),
                `Pardavė transportą ${item['name']}`,
                `Gauta į biudžetą: ${methods.moneyFormat(price)}`,
                fractionId
            );
        });
    });
    ...
    player.notify(`~b~Jus pardavete organizacijos transporta už ${methods.moneyFormat(price)}`);
});
...
mp.events.addRemoteCounted('server:fraction:vehicleBuy2', (player, id, price) => {
    ...
    if (user.getBankMoney(player) < price) {
        player.notify('~r~Banko saskaitoje neturite pakankamai pinigu');
        return;
    }

    mysql.executeQuery(`SELECT * FROM cars_fraction WHERE fraction_id = '${fractionId * -1}'`, function (err, rows2, fields) {
        if (rows2.length > 11) {
            player.notify('~r~Yra tik 10 transporto priemoniu');
            return;
        }
        ...
        mysql.executeQuery(`SELECT * FROM cars_fraction WHERE id = '${id}'`, function (err, rows, fields) {
            rows.forEach(function (item) {
                ...
                user.removeBankMoney(player, price, 'Nupirko transportą organizacijai');
                coffer.addMoney(1, price);
...
mp.events.addRemoteCounted('server:fraction:vehicleSell2', (player, id, price) => {
    ...
    user.addMoney(fractionId, price, 'Pardavė organizacijos transportą');
    coffer.removeMoney(1, price);
...
mp.events.addRemoteCounted('server:vehicle:spawnFractionCar', (player, id) => {
    ...
    player.notify('~b~Transportas stovi parkinge, imkite ji');
});
...
mp.events.addRemoteCounted('server:user:uninvite', (player, id) => {
    ...
    if (!user.isLeader(player) && !user.isSubLeader(player)) {
        player.notify('~r~Jus nesate lyderis, kuri reikia atleisti, arba lyderio pavaduotojas');
        return;
    }
    ...
    if (user.isLogin(target)) {
        ...
        user.addHistory(target, 0, 'Buvo atleistas iš organizacijos ' + methods.getFractionName(user.get(player, 'fraction_id')) + '. Atleido: ' + user.getRpName(player));
        ...
        target.notify('~r~Jus atleido is organizacijos');
        player.notify('~b~Jus atleidote darbuotoja: ~s~' + user.getRpName(target));

        user.save(target);
    }
    else {
        mysql.executeQuery(`UPDATE users SET rank1 = '0', rank_type1= '0', fraction_id = '0', is_sub_leader = '0' where id = '${id}' AND is_leader <> 1`);
        player.notify('~b~Jus atleidote darbuotoja');
        user.addHistoryById(id, 0, 'Buvo atleistas iš organizacijos ' + methods.getFractionName(user.get(player, 'fraction_id')) + '. Atleido: ' + user.getRpName(player));
    }
});
...
mp.events.addRemoteCounted('server:user:uninviteF', (player, id) => {
    ...
    if (user.isLogin(target)) {
        ...
        target.notify('~r~Jus ismete is seimos');
        player.notify('~b~Jus atleidote: ~s~' + user.getRpName(target));

        user.save(target);
    }
    else {
        mysql.executeQuery(`UPDATE users SET rankа = '0', rank_typeа = '0', family_id = '0', is_sub_leaderа = '0' where id = '${id}' AND is_leaderа <> 1`);
        player.notify('~b~Jus atleidote seimos nari');
    }
});
...
mp.events.addRemoteCounted('server:user:askSellLic', (player, id, lic, price) => {
    ...
    if (user.isLogin(target)) {
        ...
        switch (lic) {
            case 'a_lic':
                licName = 'А kategorijos licencija';
                break;
            case 'b_lic':
                licName = 'B kategorijos licencija';
                break;
            case 'c_lic':
                licName = 'C kategorijos licencija';
                break;
            case 'air_lic':
                licName = 'Oro transporto licencija';
                break;
            case 'ship_lic':
                licName = 'Vandens transporto licencija';
                break;
            case 'taxi_lic':
                licName = 'Keleivių vežimo licencija';
                break;
            case 'law_lic':
                licName = 'Teisininko licencija';
                break;
            case 'gun_lic':
                licName = 'Ginklų licencija';
                break;
            case 'biz_lic':
                licName = 'Verslo licencija';
                break;
            case 'fish_lic':
                licName = 'Žvejybos leidimas';
                break;
            case 'marg_lic':
                licName = 'Marihuanos vartojimo leidimas';
                break;
            case 'med_lic':
                licName = 'Sveikatos draudimas';
                break;
...
mp.events.addRemoteCounted('server:user:askSellRLic', (player, id, lic) => {
    ...
    if (user.isLogin(target)) {
        ...
        methods.saveFractionLog(
            ...
            `Gauta "${methods.getLicName(lic)}" pilietybė ${user.getRpName(player)}`,
            ``,
            user.get(target, 'fraction_id')
        );
        user.addHistory(player, 4, `Gauta ${methods.getLicName(lic)} (${user.getRpName(player)})`);

        player.notify('~g~Jus gavote licencija');
    }
    else {
        mysql.executeQuery(`UPDATE users SET ${lic} = '0' WHERE id ='${methods.parseInt(id)}'`);
        user.addHistoryById(id, 4, `Gauta ${methods.getLicName(lic)} (${user.getRpName(player)})`);
        player.notify('~g~Jus gavote licencija');
    }
});
...
mp.events.addRemoteCounted('server:user:buyLicensePlayer', (player, id, lic, price) => {
    ...
    if (user.isLogin(target)) {
        ...
        user.removeMoney(player, price, 'Licencijos pirkimas');
        user.addMoney(target, price * 0.2, 'Licencijos pardavimas');
        coffer.addMoney(coffer.getIdByFraction(user.get(target, 'fraction_id')), price * 0.8);

        methods.saveFractionLog(
            user.getRpName(target),
            `Išduota "${methods.getLicName(lic)}" pilietybė ${user.getRpName(player)}`,
            `Biudžeto papildymas: ${methods.moneyFormat(price * 0.8)}`,
            user.get(target, 'fraction_id')
        );

        user.giveLic(player, lic, 24, `Išdavė: ${user.getRpName(target)}`);
        player.notify('~g~Sveikiname jus isigijus ~s~' + methods.getLicName(lic));
        target.notify(`~g~Jus uždirbote ${methods.moneyFormat(price * 0.2)}\n~g~Gautas organizacijos biudžetas ${methods.moneyFormat(price * 0.8)}`);
    }
    else {
        player.notify('~r~Žaidejas nerastas');
    }
    ...
    try {
        ...
        if (user.isLogin(p)) {
            if (reason == 'clear') {
                ...
                methods.saveFractionLog(
                    user.getRpName(player),
                    `Leidimas ieškoti piliečio ${user.getRpName(p)}`,
                    ``,
                    user.get(player, 'fraction_id')
                );

                player.notify('~g~Isvalete paieska');
            }
            else {
                player.notify('~g~Paskelbete paieska');
    ...
    try {
        ...
        if (user.isLogin(p)) {
            ...
            if (user.get(p, 'jail_time') > 0) {
                player.notify('~r~Žaidejas kalejime');
                return;
            }
            coffer.addMoney(coffer.getIdByFraction(user.get(player, 'fraction_id'), 1500));
            user.addMoney(player, 1500, 'Premija');
            player.notify('~g~Jus atlikote aresta. Premija: ~s~ 1 500€ ');

            methods.saveFractionLog(
                user.getRpName(player),
                `Atliktas areštas ${user.getRpName(p)}`,
                `Gauta lėšų į biudžetą: ${methods.moneyFormat(1500)}`,
                user.get(player, 'fraction_id')
            );
    ...
    try {
        ...
        if (user.isLogin(p)) {
            ...
            if (!user.isLogin(p) || user.get(p, 'med_time') <= 0) {
                player.notify('~r~Žaidejas nera gydomas');
                return;
            }

            coffer.addMoney(coffer.getIdByFraction(user.get(player, 'fraction_id'), 400));
            user.addMoney(player, 400, 'Premija');
            player.notify('~g~Israsete žaideja. Premija: ~s~400€');
...
mp.events.addRemoteCounted('server:med:heal', (player, id) => {
    ...
    try {
        ...
        if (user.isLogin(p)) {
            ...
            player.notify('~g~Jus isgydete žaideja.');
...
mp.events.addRemoteCounted('server:user:giveSubLeader', (player, id) => {
    ...
    if (!user.isLeader(player)) {
        player.notify('~r~Jus nesate lyderis, kad suteiktumete tokius igaliojimus.');
        return;
    }
    ...
    if (user.isLogin(target)) {
        ...
        user.addHistory(target, 0, 'Išduoti pavaduotojo įgaliojimai organizacijoje ' + methods.getFractionName(user.get(player, 'fraction_id')) + '. Išdavė: ' + user.getRpName(player));

        user.set(target, 'is_sub_leader', true);

        target.notify('~g~Jums suteikti pavaduotojo igaliojimai');
        player.notify('~b~Jus suteikete pavaduotojo igaliojimus: ~s~' + user.getRpName(target));

        user.save(target);
    }
    else {
        mysql.executeQuery(`UPDATE users SET is_sub_leader = '1' where id = '${id}' AND is_leader <> 1`);
        player.notify('~b~Jus suteikete pavaduotojo igaliojimus');
        user.addHistoryById(id, 0, 'Išduoti pavaduotojo įgaliojimai organizacijoje ' + methods.getFractionName(user.get(player, 'fraction_id')) + '. Išdavė: ' + user.getRpName(player));
    }
});
...
mp.events.addRemoteCounted('server:user:giveSubLeaderF', (player, id) => {
    ...
    if (!user.isLeaderF(player)) {
        player.notify('~r~Jus nesate lyderis, kad suteiktumėte tokius igaliojimus.');
        return;
    }
...
mp.events.addRemoteCounted('server:user:giveLeader2', (player, id) => {
    ...
    if (user.isLogin(target)) {
        ...
        target.notify('~g~Jums suteiktas vadovo autoritetas');
        player.notify('~b~Jus suteikete vadovo autoriteta: ~s~' + user.getRpName(target));
        ...
        player.notify('~b~Jus suteikete vadovo autoriteta');
        user.set(player, 'is_sub_leader2', true);
        user.set(player, 'is_leader2', false);
        user.save(player);
    }
...
mp.events.addRemoteCounted('server:user:takeSubLeader', (player, id) => {
    ...
    if (user.isLogin(target)) {
        ...
        user.addHistory(target, 0, 'Panaikinti pavaduotojo įgaliojimai organizacijoje ' + methods.getFractionName(user.get(player, 'fraction_id')) + '. Panaikino: ' + user.getRpName(player));

        user.set(target, 'rank1', rank);
        user.set(target, 'rank_type1', 0);
        user.set(target, 'is_sub_leader', false);

        target.notify('~r~Is jusu buvo atimti pavaduotojo igaliojimai');
        player.notify('~b~Jus atemete pavaduotojo igaliojimus: ~s~' + user.getRpName(target));

        user.save(target);
    }
    else {
        mysql.executeQuery(`UPDATE users SET is_sub_leader = '0', rank1 = '${rank}', rank_type1 = '0' where id = '${id}' AND is_leader <> 1`);
        player.notify('~b~Jus atemete pavaduotojo igaliojimus: ~s~' + id);
        user.addHistoryById(id, 0, 'Panaikinti pavaduotojo įgaliojimai organizacijoje ' + methods.getFractionName(user.get(player, 'fraction_id')) + '. Panaikino: ' + user.getRpName(player));
    }
});
...
mp.events.addRemoteCounted('server:user:newRank', (player, id, rank) => {
    ...
    if (!user.isLeader(player) && !user.isSubLeader(player)) {
        if (user.get(player, 'rank1') >= rank) {
            player.notify('~r~Jus neturite igaliojimu paskelbti si pranesima');
            return;
        }
    }
    ...
    if (user.isLogin(target)) {
        ...
        user.addHistory(target, 0, 'Išleistas naujas pranešimas. Išleistas: ' + user.getRpName(player));

        user.set(target, 'rank1', rank);

        target.notify('~g~Jums buvo paskirtos naujos pareigos');
        player.notify('~b~Jus paskyrete naujas pareigas: ~s~' + user.getRpName(target));

        user.save(target);
    }
    else {
        mysql.executeQuery(`UPDATE users SET rank = '${rank}' where id = '${id}' AND is_leader <> 1`);
        player.notify('~b~Jus paskyrete naujas pareigas');
        user.addHistoryById(id, 0, 'Išleistas naujas pranešimas. Išleistas: ' + user.getRpName(player));
    }
});
...
mp.events.addRemoteCounted('server:user:newDep', (player, id, dep) => {
    ...
    if (user.isLogin(target)) {
        ...
        user.addHistory(target, 0, 'Buvo perkeltas į skyrių ' + depName + '. Perkėlė: ' + user.getRpName(player));

        user.set(target, 'rank1', rank);
        user.set(target, 'rank_type1', dep);

        target.notify('~g~Buvote perkeltas i kita skyriu~s~ ' + depName);
        player.notify('~b~Jus perkelete i kita skyriu: ~s~' + user.getRpName(target));

        user.save(target);
    }
    else {
        mysql.executeQuery(`UPDATE users SET rank1 = '${rank}', rank_type1 = '${dep}' where id = '${id}' AND is_leader <> 1`);
        player.notify('~b~Jus perkelete i kita skyriu');
        user.addHistoryById(id, 0, 'Buvo perkeltas į skyrių ' + depName + '. Perkėlė: ' + user.getRpName(player));
    }
});
...
mp.events.addRemoteCounted('server:user:newDep2', (player, id, dep) => {
    ...
    if (user.isLogin(target)) {
        ...
        target.notify('~g~Buvote perkeltas i kita skyriu');
        player.notify('~b~Jus perkelete i kita skyriu: ~s~' + user.getRpName(target));
...
mp.events.addRemoteCounted('server:sellVeh', (player) => {
    ...
    if (user.hasById(user.getId(player), 'grabVeh')) {
        player.notify('~r~Dabar negalite parduoti transporto');
        return;
    }
    ...
    if (user.get(player, 'fraction_id2') > 0)
        fraction.addMoney(user.get(player, 'fraction_id2'), money / 2000, `Transporto užgrobimas (${user.get(player, 'name')})`);
    ...
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        user.hideLoadDisplay(player);
        player.notify('~g~Jus uždirbote: ~s~' + methods.numberFormat(money / 1000) + 'ec');
        if (!vehicles.exists(veh))
            return;
        vehicles.respawn(veh);
    }, 1000);
    ...
    vehicles.getOccupants(veh).forEach(p => {
        if (user.isLogin(p) && (user.isTie(p) || user.isCuff(p))) {
            try {
                ...
                if (p.health <= 0) {
                    player.notify('~r~Žaidejas yra mires');
                    return;
                }
                ...
                user.addCashMoney(player, money);
                p.notify('~r~Is jusu ateme ~s~' + methods.moneyFormat(money));
                player.notify('~r~Jus atemete ~s~' + methods.moneyFormat(money));
...
mp.events.addRemoteCounted('server:sellMoney', (player) => {
    ...
    mysql.executeQuery(`SELECT * FROM items WHERE owner_id = ${user.getId(player)} AND owner_type = 1`, function (err, rows, fields) {
        ...
        if (count > 0) {
            ...
            fraction.addMoney(frId, moneyHalf * (procent / 100), 'Pinigų plovimas');

            if (procent < 100) {
                mp.players.forEach(p => {
                    if (user.isLogin(p) && user.get(p, 'fraction_id2') === frId) {
                        user.addCryptoMoney(p, moneyHalf * ((100 - procent) / 100) / currentOnline, 'Pinigų plovimas');
                        p.notify(`~g~I jusu saskaita buvo pervesta: ~s~${methods.cryptoFormat(moneyHalf * ((100 - procent) / 100) / currentOnline)}`);
                    }
                });
            }
            player.notify(`~b~Is pinigu plovimo gauti pinigai buvo padalyti taip: ${procent}% pervedama i organizacijos saskaita, ${100 - procent}% dalijamasi su visais nariais, kurie yra prisijunge.`);
        }
        else {
            player.notify('~r~Jus neturite pinigu pinigu plovimui');
        }
    });
});
...
mp.events.addRemoteCounted('server:sellMoneyBusiness', (player, bid, text) => {
    ...
    mysql.executeQuery(`SELECT * FROM items WHERE owner_id = ${user.getId(player)} AND owner_type = 1`, function (err, rows, fields) {
        ...
        if (count > 0) {
            ...
            player.notify(`~b~Pinigai buvo plaunami i verslo saskaita`);

            if (methods.getRandomInt(0, user.get(player, 'stats_darknet')) < 5)
                user.giveWanted(player, 50, 'Neteisėtas sandoris');
...
mp.events.addRemoteCounted('server:sellMoneyPolice', (player) => {
    ...
    mysql.executeQuery(`SELECT * FROM items WHERE owner_id = ${user.getId(player)} AND owner_type = 1`, function (err, rows, fields) {
        ...
        if (count > 0) {
            ...
            coffer.addMoney(coffer.getIdByFraction(frId), moneyHalf, 'Premija už nešvarius pinigus');

            mp.players.forEach(p => {
                if (user.isLogin(p) && user.get(p, 'fraction_id') === frId) {
                    user.addPayDayMoney(p, moneyHalf / currentOnline, 'Premija');
                    p.notify(`~g~I jusu paskyra buvo iskaityta: ~s~${methods.moneyFormat(moneyHalf / currentOnline)}`);
                }
            });
            player.notify('~b~Pinigai buvo padalyti taip: 50 % atitenka organizacijai, 50 % padalijama visiems nariams, kurie yra prisijunge.');
        }
        else {
            player.notify('~r~Jus neturite jokiu pagrobtu pinigu');
        }
    });
});
...
mp.events.addRemoteCounted('server:vehicle:park', (player) => {
    ...
    try {
        ...
        if (veh) {
            ...
            player.notify('~b~Pastatete transporto priemone');
        }
    }
    catch (e) {
        methods.debug(e);
...
mp.events.addRemoteCounted('server:vehicle:parkFraction', (player) => {
    ...
    try {
        ...
        if (veh) {
            ...
            if (veh.dimension > 0) {
                player.notify('~r~Draudžiama statyti transporto priemones viduje');
                return;
            }
            ...
            if (methods.distanceToPos(spawnPos, player.position) > 100) {
                player.notify('~r~Jus esate per toli nuo jusu spawno');
                return;
            }
...
mp.events.addRemoteCounted('server:vehicle:park2', (player) => {
    ...
    try {
        ...
        if (veh) {
            ...
            player.notify('~b~Sumokejote bauda, dabar jusu transporto priemone atsiras stovejimo vietoje.');
        }
    }
    catch (e) {
        methods.debug(e);
...
mp.events.addRemoteCounted('server:vehicle:ejectById', (player, id) => {
    ...
    vehicles.getOccupants(player.vehicle).forEach(p => {
        if (user.isLogin(p) && p.id === id) {
            p.notify('~r~Buvote ismestas is transporto');
            p.removeFromVehicle();
        }
    })
});
                ...
                try {
                    if (user.isLogin(p) && p.id == id) {
                        ...
                        if (p.health > 1 && !user.isCuff(p) && !user.isTie(p)) {
                            player.notify('~r~Žaidejas turi buti surakintas antrankiais, suristas arba mires');
                            return;
                        }
...
mp.events.addRemoteCounted('server:vehicle:radarCheck', (player, ticketSpeed, ticketPrice, number, vid) => {
    ...
    try {
        ...
        if (user.isLogin(driver)) {
            user.addMoney(driver, ticketPrice / 2, 'Radaro montavimas');
            driver.notify("~y~Transportas ~s~" + number + "~y~ užfiksuotas greicio matuokliu ~s~" + ticketSpeed + "km/h~y~, transporto priemonei buvo skirta bauda, lygi ~s~" + methods.moneyFormat(ticketPrice));
            driver.notify("~y~Turite teise i tokio dydžio premija. ~s~" + methods.moneyFormat(ticketPrice / 2));
        }
    }
    catch (e) {
...
mp.events.addRemoteCounted('server:vehicle:removeNumber', (player, vid) => {
    ...
    try {
        ...
        if (vehicles.exists(veh)) {
            if (user.getCashMoney(player) < 10000) {
                player.notify('~r~Jus neturite 10000€ rankose');
                return;
            }
            if (veh.getVariable('user_id') > 0) {
                let frId = user.get(player, 'fraction_id2');
                user.removeCashMoney(player, 10000, 'Numerio ženklo nuėmimas nuo transporto priemonės');
                if (frId > 0) {
                    fraction.addMoney(frId, 5, 'Pelnas iš numerio ženklų nuėmimo');
                }
                veh.numberPlate = '';
                player.notify('~y~Dabar už si transporta baudos nebus skiriamos');
                vehicles.set(veh.getVariable('container'), 'removeNumber', true);
            }
            else {
                player.notify('~y~Transportas turi buti privatus');
            }
        }
    }
    catch (e) {
            ...
            mysql.executeQuery(`SELECT bonus, bonus2, is_one, is_use FROM promocode_list WHERE code = '${promocode}' LIMIT 1`, function (err, rows, fields) {
                if (rows.length >= 1) {
                    rows.forEach(row => {
                        ...
                        if (row['is_use']) {
                            player.notify(`~g~Promo kodas buvo aktyvuotas`);
                            return;
                        }

                        if (methods.parseInt(row['bonus']) > 0) {
                            user.addMoney(player, methods.parseInt(row['bonus']));
                            player.notify(`~g~Promo kodas: ${promocode} aktyvuotas, jus gavote $${methods.numberFormat(row['bonus'])}`);
                        }
                        ...
                        if (rows.length >= 1) {
                            if (user.get(player, 'promocode') === '') {
                                if (user.get(player, 'online_time') < 169) {
                                    ...
                                    let string = `~b~Jūs įvedėte promo kodą: ~s~${promocode}\n`;
                                    if (paramsStart.money > 0)
                                        string += `~b~Jūs gavote~s~ ${methods.moneyFormat(paramsStart.money)}\n`;
                                    if (paramsStart.vipt === 1)
                                        string += `~b~Jūs gavote ~s~VIP LIGHT~b~~s~${paramsStart.vip}d.\n`;
                                    if (paramsStart.vipt === 2)
                                        string += `~b~Jūs gavote ~s~VIP HARD~b~~s~${paramsStart.vip}d.\n`;
                                ...
                                player.notify("~r~Žaidete ilgiau nei 24 valandas, promo kodo nera");
                                return;
                            }
                            player.notify("~r~Si promokoda galite aktyvuoti tik tada, kai registruojate savo simboli.");
                        } else {
                            player.notify("~r~Tokio promo kodo nera");
                        }
                    });
                }
            });
        } else {
            player.notify("~r~Jus jau aktyvavote promo koda");
        }
    });
});
    ...
    if (user.isLogin(player)) {
        ...
        try {
            if (user.isCuff(player)) {
                user.addHistory(player, 1, 'Buvo įkalintas');
                user.warn(player, 1, 'Išėjimas iš žaidimo suėmimo metu');
        ...
        try {
            if (user.isTie(player)) {
                user.addHistory(player, 1, 'Buvo įkalintas');
                user.warn(player, 1, 'Išėjimas iš žaidimo pagrobimo metu');
                user.toLspdSafe(player);
                /*user.set(player, 'jail_time', 120 * 60);
                user.set(player, 'wanted_level', 0);
    ...
    if (user.isLogin(player)) {
        ...
        if (user.has(player, 'duelTarget'))
        {
            ...
            if (countDeath === 0) {
                ...
                setTimeout(function () {
                    try {
                        player.outputChatBoxNew(`!{#FF9800}JŪS PRALAIMĖJOTE DVIKOVĄ`);
                        ...
                        if (user.isLogin(target)) {
                            ...
                            target.outputChatBoxNew(`!{#4CAF50}JŪS LAIMĖJOTE DVIKOVĄ`);
                            ...
                            if (bet > 0 && user.has(target, 'duelBet')) {
                                user.addCashMoney(target, bet * 2, 'Pergalė dvikovoje');
                            }
            ...
            if (user.isLogin(target)) {
                ...
                player.outputChatBoxNew(`!{#03A9F4}ROUNDO PRADŽIA`);
                target.outputChatBoxNew(`!{#03A9F4}ROUNDO PRADŽIA`);

                user.duelTimer(player);
                user.duelTimer(target);

                player.outputChatBoxNew(`!{#f44336}Jums liko mirčių:!{#FFFFFF} ${user.get(player, 'duelCount')}`);
                player.outputChatBoxNew(`!{#f44336}Priešininkui liko mirčių:!{#FFFFFF} ${user.get(target, 'duelCount')}`);
                target.outputChatBoxNew(`!{#f44336}Jums liko mirčių:!{#FFFFFF} ${user.get(target, 'duelCount')}`);
                target.outputChatBoxNew(`!{#f44336}Priešininkui liko mirčių:!{#FFFFFF} ${user.get(player, 'duelCount')}`);
            }
            else {
                if (bet > 0)
                    user.addCashMoney(player, bet * 2, 'Pergalė dvikovoje');
                user.reset(player, 'duelBet');
                user.reset(player, 'duelTarget');
                user.reset(player, 'duelCount');
                player.dimension = 0;

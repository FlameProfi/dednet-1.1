...
let host = 'localhost';
let dbuser = 'angelas';
let database = 'Serveris';
let password = 'Angelas942001312407405';
//let password = 'b3282a2f2a28757b3a18ab833de16a9c54518c0b0cf493e3f0a7cf09386f326a';

const pool = mysql2.createPool({
    host: host,
    //socketPath: '/run/mysqld/mysqld.sock', //Если хочешь напрямую по сокету подключать, ускоряет немного всю хуйню
    user: dbuser,
    password: password,
    database: database,
    port: 3306,
                ...
                if (type === eventTypes.Other) {
                    switch (action) {
                        ...
                        case "SaveAll":
                            ...
                            break;
                        case "ChatNotify":
                            chat.sendToAll('Administracija', param['text']);
                            break;
                    }
                }
                    ...
                    switch (action) {
                        ...
                        case "AnswerReport":
                        {
                            mysql.executeQuery(`DELETE FROM game_event WHERE id = '${id}'`);
                            if (player == null) break;
                            player.outputChatBoxNew(`[${chat.getTime()}] !{#f44336}Atsakymas iš administratorių ${param['name']}:!{#FFFFFF} ${param['text']}`);
                            break;
                        }
                        case "AnswerAsk":
                        {
                            mysql.executeQuery(`DELETE FROM game_event WHERE id = '${id}'`);
                            if (player == null) break;
                            player.outputChatBoxNew(`[${chat.getTime()}] !{#FFC107}Atsakymas į skundą ${param['name']}:!{#FFFFFF} ${param['text']}`);
                            break;
                        }
                        case "Notify":
                        {
                        ...
                        case "AddMoney":
                        {
                            ...
                            user.addCashMoney(player, money, param["text"]);
                                player.notify(`~g~Jums buvo iskaityta ~s~${methods.moneyFormat(money)}`);
                            mysql.executeQuery(`DELETE FROM game_event WHERE id = '${id}'`);
                            break;
                        }
                        case "RemoveMoney":
                        {
                            ...
                            user.removeCashMoney(player, money, param["text"]);
                                player.notify(`~g~Is saskaitos buvo nurasyta ~s~${methods.moneyFormat(money)}`);
                            mysql.executeQuery(`DELETE FROM game_event WHERE id = '${id}'`);
                            break;
                        }
                    }

...
user.createAccount = function(player, login, pass, email) {
    ...
    user.doesExistAccount(login, email, player.socialClub, function (cb) {
        if (cb == 1) {
            user.showCustomNotify(player, 'Paskyra su siuo SocialClub jau yra', 1);
            return;
        }
        else if (cb == 2) {
            user.showCustomNotify(player, 'Vartotojo vardas jau užimtas', 1);
            return;
        }
        else if (cb == 3) {
            user.showCustomNotify(player, 'El. pastas jau užimtas', 1);
            return;
        }
...
user.createUser = function(player, name, surname, age, promocode, referer, national) {
    ...
    user.doesExistUser(name + ' ' + surname, function (cb) {
        ...
        if (cb == true) {
            user.showCustomNotify(player, 'Vardas ir pavarde jau užimti kito naudotojo, pabandykite kita', 1);
            return;
        }
        ...
        mysql.executeQuery(`SELECT * FROM promocode_top_list WHERE promocode = '${promocode}' AND is_use = 0 LIMIT 1`, function (err, rows, fields) {
            if (rows.length >= 1) {
                ...
                user.showCustomNotify(player, 'Prasome palaukti...');
                ...
                user.showCustomNotify(player, 'Promo kodas nerastas. Promo koda galima ivesti po registracijos per 24 valandas per M - Nustatymai - Promo kodas', 1);
            }
        });
    });
};
...
user.loginAccount = function(player, login, pass) {
    ...
    user.validateAccount(player, login, pass, function (callback) {
        ...
        if (callback == false) {
            user.showCustomNotify(player, 'Slaptažodžio klaida arba paskyra dar nesukurta', 1);
            return;
        }
        ...
        if (mp.players.exists(player))
        {
                    ...
                    if (err) {
                        ...
                        rows.forEach(row => {
                            try {
                                ...
                                if (row['pos_x'] !== 0)
                                    spawnList.push('Išėjimo taškas');

                                if (row['house_id'])
                                    spawnList.push('Namas');

                                if (row['condo_id'])
                                    spawnList.push('Butas');

                                if (row['apartment_id'])
                                    spawnList.push('Apartamentai');

                                if (row['yacht_id'])
                                    spawnList.push('Jachta');

                                if (row['stock_id']) {
                                    if (stocks.get(row['stock_id'], 'upgrade_g'))
                                        spawnList.push('Sandėlis');
                                }

                                if (fraction.canSpawn(row['fraction_id2']) && row['fraction_id2'] > 0)
                                    spawnList.push('Organizacijos spawnas');
                                if (row['fraction_id'] === 4) {
                                    spawnList.push('Spawnas barakuose');
                                    spawnList.push('Spawnas lėktuvnešyje');
                                }

                                spawnList.push('Standartas');
            ...
            user.showCustomNotify(player, 'Ivyko nežinoma klaida. Klaidos kodas #9999', 1);
        //player.notify('~b~Входим в аккаунт...');
    });
};

user.loginUser = function(player, name, spawn = 'Standartas') {
    ...
    user.validateUser(name, function (callback) {
        ...
        if (callback == false) {
            user.showCustomNotify(player, 'Paskyros autorizavimo klaida, bandykite dar karta', 1);
            return;
        }
...
user.loadUser = function(player, name, spawn = 'Standartas') {
    ...
    mysql.executeQuery(`SELECT ${selectSql} FROM users WHERE name = ? LIMIT 1`, name, function (err, rows, fields) {
        ...
        setTimeout(function() {
            ...
            try {
                ...
                user.showCustomNotify(player, 'Ivyko klaida ', 1);
                user.kick(player, 'Įvyko stebuklinga klaida, žurnalai buvo išsiųsti kūrėjams, tiesiog iš naujo įveskite');
                return;
            }
            ...
            if (user.get(player, 'date_ban') > methods.getTimeStamp()) {
                ...
                user.showCustomNotify(player, 'Jus užblokuotas, žr. issamia informacija svetaineje: revolutionrp.lt', 1, 5, 60000);
                user.kick(player, 'Jūs užblokuotas');
                return;
            }

            if (user.get(player, 'is_online') == 1 && !mysql.isTestServer()) {
                user.resetAll(player);
                user.showCustomNotify(player, 'Vartotojas jau patvirtintas', 1);
                user.kick(player, 'Jūs buvote iškickintas');
                return;
            }
            ...
            setTimeout(function () {
                ...
                if (user.get(player, 'vip_time') > 0 && user.get(player, 'vip_time') < methods.getTimeStamp()) {
                    player.outputChatBox(`!{#f44336}Jūsų VIP statuso galiojimo laikas baigėsi`);
...
user.spawnByName = function(player, spawn = 'Standartas') {
    ...
    setTimeout(function () {
        ...
        try {
            ...
            else if (spawn == 'Organizacijos spawnas') {
...
user.getSexName = function(player) {
    if (!user.isLogin(player))
        return false;
    return user.getSex(player) == 1 ? 'Moteriška' : 'Vyriška';
};
...
user.addMoney = function(player, money, text = 'Finansinė operacija', payType = 0) {
    if (payType === 1)
...
user.removeMoney = function(player, money, text = 'Finansinė operacija', payType = 0) {
    if (payType === 1)
...
user.addCashMoney = function(player, money, text = 'Finansinė operacija') {
...
user.removeCashMoney = function(player, money, text = 'Finansinė operacija') {
...
user.addCryptoMoney = function(player, money, text = 'Finansinė operacija') {
...
user.removeCryptoMoney = function(player, money, text = 'Finansinė operacija') {
...
user.addBankMoney = function(player, money, text = "Sąskaitos operacija") {
...
user.removeBankMoney = function(player, money, text = "Sąskaitos operacija") {
...
user.setWorkExp = function(player, rep) {
    if (rep >= user.getWorkLvl(player) * 500) {
        ...
        try {
            ...
            user.sendSmsBankOperation(player, 'Priemokos už tarnybos stažą įskaitymas: ~g~€' + methods.numberFormat(sum));
        }
        catch (e) {
...
user.sendSmsBankOperation = function(player, text, title = 'Sąskaitos operacija', pref = 0) {
...
user.kick = function(player, reason, title = 'Jūs buvote iškickintas.') {
    if (!mp.players.exists(player))
        return;
    methods.debug('user.kick ' + player.socialClub + ' ' + reason);
    player.outputChatBox('!{f44336}' + title);
    player.outputChatBox('!{f44336}Priežastis: !{FFFFFF}' + reason);
...
user.kickAntiCheat = function(player, reason, title = 'Jūs buvote iškickintas.') {
    methods.debug('user.kickAntiCheat');
    if (user.isLogin(player)) {
        chat.sendToAll('Anti-Cheat Protection', `${user.getRpName(player)} (${player.id})!{${chat.clRed}} buvo iškickintas su priežastimi!{${chat.clWhite}} ${reason}`, chat.clRed);
        discord.sendDeadList(user.getRpName(player), 'Buvo iškickintas', reason, 'Anti-Cheat Protection');
    }
...
user.duelTimer = function(player) {
    ...
    player.notify('~g~Startas už 5 sek.');
    ...
    setTimeout(function () {
        try {
            player.notify('~g~Startas už 4 sek.');
        }
        catch (e) {
    ...
    setTimeout(function () {
        try {
            player.notify('~g~Startas už 3 sek.');
        }
        catch (e) {
    ...
    setTimeout(function () {
        try {
            player.notify('~g~Startas už 2 sek.');
        }
        catch (e) {
    ...
    setTimeout(function () {
        try {
            player.notify('~g~Startas už 1 sek.');
        }
        catch (e) {
    ...
    setTimeout(function () {
        try {
            player.notify('~g~Startas nuo GO');
        ...
        try {
            ...
            if (user.has(player, 'job_' + skillPostfix)) {
                ...
                if (currentSkill == skillCount - 1) {
                    ...
                    chat.sendToAll('Valstybės naujienos', `${user.getRpName(player)} !{${chat.clBlue}} tapo vienu iš geriausių San Andreas valstijos darbuotojų ir gavo atlygį ${methods.moneyFormat(skillWin)}`, chat.clBlue);
                    user.addMoney(player, skillWin);
                    user.save(player);
                }
                else {
                    user.set(player, 'job_' + skillPostfix, currentSkill + 1);
                    player.notify('~g~Jusu profesijos igudžiai buvo patobulinti');
                }
            }
        }
        catch (e) {}
...
user.giveJobMoney = function(player, money, jobId = 0) {
    ...
    if (user.hasById(user.getId(player), 'uniform')) {
        player.notify('~r~Negalite dirbti su uniforma');
        return;
    }
    ...
    try {
        ...
        if (jobItem[5] > 0 && user.get(player, 'job_' + jobItem[4]) > 0) {
            ...
            desc = `\n~y~Padidinti ${methods.parseFloat((user.get(player, 'job_' + jobItem[4]) / jobItem[5]) * 100).toFixed(2)}% darbo užmokestį už įgytą įgūdį.`
        }
    }
    catch (e) {}
    ...
    if (user.get(player, 'bank_card') < 1) {
        ...
        player.notify('Jus uždirbote: ~g~' + methods.moneyFormat(money) + desc);
    }
    else {
        user.addBankMoney(player, money);
        user.sendSmsBankOperation(player, `Lėšų įskaitymas: ~g~${methods.moneyFormat(money)}` + desc);
    }
...
user.arrest = function(player) {
    ...
    user.addHistory(player, 1, 'Buvo pasodintas į kalėjimą ' + user.get(player, 'wanted_level') + ' metų');
    user.jail(player, methods.parseInt(user.get(player, 'wanted_level')) * 120);

    user.set(player, 'st_jail', user.get(player, 'st_jail') + 1);
};

user.giveWanted = function(player, level, reason, officer = 'Sistema') {
    ...
    if (reason == 'clear') {
        ...
        user.addHistory(player, 1, `Atlikta paieška (${officer})`);
    }
    else {
        ...
        player.notifyWithPicture('Pranesimas', 'Police Department', 'Prasome pranesti Los Santos policijos departamentui', 'WEB_LOSSANTOSPOLICEDEPT', 2);

        user.addHistory(player, 1, `Paskelbta paieška ${level}. Priežastis: ${reason}. (${officer})`);
...
user.giveLic = function (player, lic, monthEnd = 12, desc = '') {
    ...
    switch (lic) {
        ...
        case 'c_lic':
            ...
            break;
        case 'air_lic':
            licName = 'oro transportui';
            break;
        case 'ship_lic':
            licName = 'vandens transportui';
            break;
        case 'taxi_lic':
            licName = 'keleivių vežimui';
            break;
        case 'law_lic':
            licName = 'teisininkui';
            break;
        ...
        case 'fish_lic':
            ...
            break;
        case 'marg_lic':
            licName = 'marihuanos vartojimui';
            break;
    }

    if (lic == 'med_lic') {
        player.notify(`~g~Jus gavote ~s~sveikatos draudima~g~~s~${monthEnd} ~g~men.`);
        user.addHistory(player, 4, `Gavo sveikatos draudimą ${monthEnd} mėn. ` + desc);
        return;
    }
    player.notify(`~g~Jus gavote licencija ~s~${licName}~g~~s~${monthEnd} ~g~men.`);
    user.addHistory(player, 4, `Gavo licenciją ${licName}${monthEnd} mėn. ` + desc);
};
...
user.buyLicense = function (player, type, price, month, typePay = 0) {
    ...
    try {
        ...
        {
            player.notify('~r~Jus neturite registracijos');
            return;
        }
        ...
        if (!user.get(player, type))
        {
            if (typePay === 1) {
                ...
                user.removeBankMoney(player, price, 'Licencijos pirkimas');
            }
            else {
        ...
        player.notify("~r~Jus jau turite sia licencija");
    }
    catch (e) {
...
user.toLspdSafe = function(player, time = 28, target = null) {
    if (!mp.players.exists(player))
        return false;
    user.showCustomNotify(player, 'Jusu ginklai guli LSPD/BCSD seife');
    ...
    if (user.isLogin(target)) {
        mysql.executeQuery(`SELECT * FROM items WHERE owner_id = ${user.getId(player)} AND owner_type = 1`, function (err, rows, fields) {
            ...
            if (count > 0) {
                ...
                target.notify('~b~Pinigai buvo padalyti taip. 50 % atitenka organizacijai, 50 % padalijama visiems tinklo nariams.');
            }
        });
    }
    else
...
user.createBlip = function(player, id, x, y, z, blipId = 1, blipColor = 0, route = false, shortRange = false, name = 'Blip', rot = 0, scale = 0.8) {
    if (!mp.players.exists(player))
        return false;
...
user.payDay = async function (player) {
    ...
    if (user.get(player, 'warns') > 0) {
        ...
        if (user.get(player, 'online_warn') >= 508) {
            ...
            player.notify(`~g~Serveris atsauke 1 ispejima`);
        }
    }
    ...
    if (user.get(player, 'online_time') === 169) {
        if (user.get(player, 'referer') !== "") {
            ...
            user.addCashMoney(player, 20000, 'Valstybės premija');
            player.notify(`~g~Per rekomendaciju sistema gavote 20 000€`);
            player.notify(`~g~Kvieciant ${user.get(player, 'referer')} gavo 500bp i asmenine saskaita`);
        ...
        if (user.get(player, 'promocode') !== "") {
            ...
            mysql.executeQuery(`SELECT * FROM promocode_top_list WHERE promocode = '${promocode}' LIMIT 1`, function (err, rows, fields) {
                if (rows.length >= 1) {
                    ...
                    let string = `~b~Projekte praleidote 24 valandas, gavote premija pagal promo koda ${promocode}\n`;
                    if (paramsStart.money > 0)
                        string += `~b~Jus gavote~s~ ${methods.moneyFormat(paramsStart.money)}\n`;
                    ...
                    user.addCashMoney(player, paramsStart.money, 'Valstybės premija');
                } else {
                    player.notify(`~g~Jus gavote ~s~24000€ ~g~is promo kodo ~s~${user.get(player, 'promocode')}`);
                    user.addCashMoney(player, 24000, 'Valstybės premija');
                }
    ...
    if (user.get(player, 'bank_card') > 0) {
        ...
        if (player.getVariable('isAfk') === true) {
            player.notify('~r~Jums nebuvo sumoketa, nes esate AFK');
        }
            ...
            if (!user.hasById(user.getId(player), 'uniform') && !user.isNews(player) && !user.isFib(player))
            {
                ...
                if (currentCofferMoney < nalog) {
                    user.sendSmsBankOperation(player, `~r~Organizacijos biudžete nepakanka lesu atlyginimams moketi`, 'Užmokestis');
                }
                else {
                    ...
                    if (ben > 0)
                        user.sendSmsBankOperation(player, `Registracija: ~g~${methods.moneyFormat(nalog)}\n~s~Darbo užmokescio padidinimas: ~g~${methods.moneyFormat(ben)}${desc}`, 'Užmokestis');
                    else
                        user.sendSmsBankOperation(player, `Registracija: ~g~${methods.moneyFormat(nalog)}${desc}`, 'Užmokestis');
            ...
            if (currentCofferMoney < sum) {
                player.notify('~r~Valstybes biudžete nepakanka pinigu ismokoms moketi');
                user.sendSmsBankOperation(player, `~r~Valstybes biudžete nepakanka pinigu ismokoms moketi`, 'Vadovas');
            }
            else {
                user.sendSmsBankOperation(player, `Registracija: ~g~${methods.moneyFormat(sum)}`, 'Vadovas');
                user.addBankMoney(player, sum, 'Nedarbo išmoka');
        ...
        if (player.getVariable('enableAdmin')) {
            /*if (player.getVariable('isAfk') === true) {
                ...
                user.sendSmsBankOperation(player, `Registracija: ~g~${methods.moneyFormat(sum)}`, '~r~Admin užmokestis');
                user.addBankMoney(player, sum, 'Nedarbo išmoka');
            }*/
        }
    }
    else {
        player.notify(`~y~Kreipkites del banko korteles`);
    }
    return true;
};
...
user.warn = function(player, count, reason, notify = true) {
    ...
    if (notify) {
        discord.sendDeadList(user.getRpName(player), 'Buvo išduotas įspėjimas', reason, 'Anti-Cheat Protection');
        chat.sendToAll(`Anti-Cheat Protection`, `${user.getRpName(player)}!{${chat.clRed}} buvo išduotas įspėjimas su priežastimi!{${chat.clWhite}} ${reason}`, chat.clRed);
    }

    if (count < 0) {
        mysql.executeQuery(`INSERT INTO ban_list (ban_from, ban_to, count, datetime, reason) VALUES ('Server', '${user.getRpName(player)}', 'Įspėjimo nuėmimas', '${methods.getTimeStamp()}', '${reason}')`);
        ...
        mysql.executeQuery(`INSERT INTO ban_list (ban_from, ban_to, count, datetime, reason) VALUES ('Server', '${user.getRpName(player)}', 'Įspėjimai', '${methods.getTimeStamp()}', '${reason}')`);
        ...
        if (user.get(player, 'warns') < 2) {
            ...
            user.set(player, 'warns', 0);
            user.ban(player, 6, 'Buvo užblokuotas (3 įspėjimai)')
        }
    }
};
...
user.giveVip = function(player, days = 7, type = 2, withChat = false, desc = '') {
    ...
    player.notify(`~g~Jums buvo isduotas VIP HARD~s~${days}d.`);
    if (withChat)
        chat.sendToAll(`Server`, `${user.getRpName(player)} (${player.id})!{${chat.clBlue}} gavo{desc}!{${chat.clWhite}} VIP HARD ${days}d.`, chat.clBlue);
};
...
user.giveMask = function(player, maskId, withChat = false, desc = '') {
    ...
    player.notify(`~g~Jus gavote kauke "${itemName}", ji yra jusu inventoriuje.`);

    if (withChat)
        chat.sendToAll(`Server`, `${user.getRpName(player)} (${player.id})!{${chat.clBlue}} laimėjo kaukę${desc}!{${chat.clWhite}} ${itemName}!{${chat.clBlue}} retumas!{${chat.clWhite}} ${methods.getRareName(mask[14])}`, chat.clBlue);
};
...
user.giveVehicle = function(player, vName, withDelete = 1, withChat = false, desc = '', isBroke = false, isFree = false, slot = 10) {
    ...
    if (user.get(player, 'car_id' + slot) === 0 && vInfo.display_name !== 'Unknown') {
        ...
        player.notify(`~g~Jus gavote automobili ${vInfo.display_name}.`);
        if (withChat)
            chat.sendToAll(`Server`, `${user.getRpName(player)} (${player.id})!{${chat.clBlue}} laimėjo transportą${desc}!{${chat.clWhite}} ${vInfo.display_name}`, chat.clBlue);
    }
    else {
        user.addMoney(player, vInfo.price, 'Prizo laimėjimas');
        player.notify(`~g~Kadangi turejote užimta ${slot} slota, skyreme jums pinigini priza, kurio suma ~s~${methods.moneyFormat(vInfo.price)}.`);
        user.save(player);
    }
};

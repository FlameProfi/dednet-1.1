...
houses.loadAll = function() {
    ...
    mysql.executeQuery(`SELECT * FROM houses`, function (err, rows, fields) {
        rows.forEach(function(item) {
            ...
            methods.createCp(hBlip.position.x, hBlip.position.y, hBlip.position.z, "Paspauskite ~g~Е~s~ jog atidarytumėte meniu");

            if (hBlip.g1.int >= 0)
                methods.createCpVector(hBlip.g1.position, "Paspauskite ~g~Е~s~ jog atidarytumėte garažo meniu", 3, -1, [33, 150, 243, 0]);
            if (hBlip.g2.int >= 0)
                methods.createCpVector(hBlip.g2.position, "Paspauskite ~g~Е~s~ jog atidarytumėte garažo meniu", 3, -1, [33, 150, 243, 0]);
            if (hBlip.g3.int >= 0)
                methods.createCpVector(hBlip.g3.position, "Paspauskite ~g~Е~s~ jog atidarytumėte garažo meniu", 3, -1, [33, 150, 243, 0]);
    ...
    houses.interiorList.forEach(function(item) {
        ...
        let y = item[1];
        let z = item[2];
        methods.createCp(x, y, z, "Paspauskite~g~Е~s~ jog atidarytumėte meniu");
    });
    ...
    houses.garageList.forEach(function(item) {
        ...
        let y = item[1];
        let z = item[2];
        methods.createCp(x, y, z, "Paspauskite~g~Е~s~ jog atidarytumėte garažo meniu", 3, -1, [33, 150, 243, 0]);
...
houses.loadLast = function() {
    ...
    mysql.executeQuery(`SELECT * FROM houses ORDER BY id DESC LIMIT 1`, function (err, rows, fields) {
        ...
        rows.forEach(function(item) {
            ...
            let id = item['id'];
            discord.sendMarketProperty(`Namas #${houses.get(id, 'number')}`, `Adresas: ${houses.get(id, 'address')} / ${houses.get(id, 'street')} #${houses.get(id, 'number')}\nValstybinė kaina: ${methods.moneyFormat(houses.get(id, 'price'))}\nGyvenamos vietos: ${houses.get(id, 'max_roommate')}`);

            chat.sendToAll(`Pridėto namo ID: ${item['id']}. Name: ${item['number']}. Int: ${item['interior']}. Price: ${methods.moneyFormat(item['price'])}`);
...
houses.insert1 = function(player, id, int, gx, gy, gz, grot) {
    ...
    player.outputChatBox(`Namas buvo sėkmingai renovuotas (G1 | ID ${id} | INT ${int})`);
};
...
houses.insert2 = function(player, id, int, gx, gy, gz, grot) {
    ...
    player.outputChatBox(`Namas buvo sėkmingai renovuotas (G2 | ID ${id} | INT ${int})`);
};
...
houses.insert3 = function(player, id, int, gx, gy, gz, grot) {
    ...
    player.outputChatBox(`Namas buvo sėkmingai renovuotas (G3 | ID ${id} | INT ${int})`);
};
...
houses.sell = function (player) {
    ...
    user.addMoney(player, nalog, 'Namo pardavimas ' + hInfo.get('address') + ' №' + hInfo.get('number'));

    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        user.addHistory(player, 3, 'Pardavė namą ' + hInfo.get('address') + ' №' + hInfo.get('number') + '. Kaina: ' + methods.moneyFormat(nalog));
        player.notify(`~g~Jus pardavete nekilnojama turta\nMokestis:~s~ ${coffer.getTaxProperty()}%\n~g~Gauta:~s~ ${methods.moneyFormat(nalog)}`);
        user.save(player);
    }, 1000);
};
...
houses.buy = function (player, id) {
    ...
    if (user.get(player, 'house_id') > 0) {
        player.notify('~r~Jus turite nekilnojamojo turto');
        return false;
    }
    if (hInfo.get('price') > user.getMoney(player)) {
        player.notify('~r~Jums neužtenka lesu');
        return false;
    }
    if (hInfo.get('user_id') > 0) {
        player.notify('~r~Nekilnojamasis turtas jau nupirktas');
        return false;
    }
    ...
    user.removeMoney(player, hInfo.get('price'), 'Namo pirkimas ' + hInfo.get('address') + ' №' + hInfo.get('number'));
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        user.addHistory(player, 3, 'Nusipirko namą ' + hInfo.get('address') + ' №' + hInfo.get('number') + '. Kaina: ' + methods.moneyFormat(hInfo.get('price')));
        user.save(player);
        player.notify('~g~Sveikiname nusipirkus nekilnojamaji turta!');
    }, 500);
    return true;
};
...
houses.enterv = function (player, id) {
    ...
    if (vehicles.exists(player.vehicle)) {
        ...
        {
            player.notify('~r~Sios klases automobiliams draudžiama ivažiuoti i garaža');
            return;
        }
    }
    ...
    if (garageId == -1) {
        player.notify('~r~Jus nuklydote per toli');
        return;
    }
    ...
    if (vehicles.exists(v) && player.vehicle) {
        ...
        if (vehicles.exists(v2) && player.vehicle) {
            player.notify('~r~Deja, dabar prie vartu stovi transporto priemone, kuria reikia atitraukti.');
            return;
        }

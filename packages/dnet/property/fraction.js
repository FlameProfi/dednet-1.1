    ...
    mysql.executeQuery(`SELECT * FROM fraction_list`, function (err, rows, fields) {
        rows.forEach(function(item) {
            ...
            if (item['is_war']) {
                ...
                methods.createBlip(new mp.Vector3(item['spawn_x'], item['spawn_y'], item['spawn_z']), 310, color, 0.6, 'Titulinė ter.');
            }
            else if (item['is_mafia']) {
                //SKIP
            }
            else if (item['spawn_x'] !== 0) {
                methods.createBlip(new mp.Vector3(item['spawn_x'], item['spawn_y'], item['spawn_z']), 565, 37, 0.6, 'Spawn org.');
            }
        });
        count = rows.length;
...
fraction.removeTaxAndSave = function() {
    ...
    mysql.executeQuery(`SELECT * FROM fraction_list`, function (err, rows, fields) {
        rows.forEach(function(item) {
            ...
            if (item['is_shop'])
                sum += 15;
            fraction.removeMoney(item['id'], sum, 'Mokestis už buvimą');
...
fraction.createCargoWar = function(count = 3) {
    ...
    methods.notifyWithPictureToFractions2('Kova del krovinio', `~r~DEMESIO!`, 'Prasidejo karas del krovinio, krovinys pažymetas žemelapyje');
    isCargo = true;
...
fraction.spawnNearCargo = function(player, isDrug = false, name = 'Speedo4', count = 9) {
    ...
    player.notify('~g~Ant furgono pritvirtinta žyma');
...
fraction.spawnNearBank = function(player, type = 0) {
    ...
    user.setWaypoint(player, posVeh.x, posVeh.y);
    player.notify('~g~Ant transporto priemones uždeta žyma');
...
fraction.spawnNearCanabis = function(player) {
    ...
    if (countZones === 0) {
        player.notify('~g~Jus neturite okupuotu teritoriju');
        return;
    }
...
fraction.createCargoArmyWar = function() {
    if (isCargoArmy)
        return;
    methods.notifyWithPictureToFractions2('Kova del armijos krovinio', `~r~DEMESIO!`, 'Prasidejo karas del armijos krovinio, krovinys netrukus bus pažymetas žemelapyje');
    isCargoArmy = true;
...
fraction.getShopGang = function(player) {
    ...
    if (!fraction.get(frId, 'is_shop')) {
        player.notify('~r~Nepavyko pagerinti situacijos del vagysciu is parduotuviu');
        return;
    }

    if (fraction.get(frId, 'cantGrab2')) {
        player.notify('~r~Siandien jau ivykdete apiplesima');
        return;
    }
    ...
    if (fraction.get(frId, 'currentGrabShop')) {
        player.notify('~r~Jus neseniai gavote arbatpinigiu parduotuvėje');
        return;
    }
    ...
    if (dateTime.getHours() < 17) {
        player.notify('~r~Galimybe naudotis tik nuo 17 iki 24 val. vakaro OOC laiku');
        return;
    }
    ...
    player.notify('~b~Lamaras sumazino parduotuveje esanciu prekiu kaina');
...
fraction.startGrabShopGang = function(player, itemId = 0) {
    ...
    let frId = user.get(player, 'fraction_id2');
    if (frId < 1) {
        player.notify('~r~Jus nepriklausote organizacijai');
        return;
    }

    if (!fraction.has(frId, 'currentGrabShop')) {
        player.notify('~r~Užduotis turi buti pradeta');
        return;
    }
    ...
    shopItem.pos.forEach((pos, i) => {
        if (methods.distanceToPos(new mp.Vector3(pos[0], pos[1], pos[2]), player.position) < 1.5) {
            ...
            if (fraction.has(frId, 'currentGrabShopActive' + i)) {
                player.notify('~r~I kasos aparata isilaužiama');
                return;
            }
            if (fraction.get(frId, 'currentGrabShop' + i)) {
                player.notify('~r~I sia kasa jau buvo isilaužta');
                return;
            }
            if (player.vehicle) {
                player.notify('~r~Jus esate transporte');
                return;
            }
            let dateTime = new Date();
            if (dateTime.getHours() >= 1 && dateTime.getHours() < 6) {
                player.notify('~r~Laikas baigesi, dabar neimanoma apiplesti parduotuves');
            ...
            dispatcher.sendLocalPos('Kodas 0', `Skubiai visiems patruliams, vyksta vagystė iš parduotuvės ${shopItem.name}`, player.position, 2);
            dispatcher.sendLocalPos('Kodas 0', `Skubiai visiems patruliams, vyksta vagystė iš parduotuvės ${shopItem.name}`, player.position, 3);
            dispatcher.sendLocalPos('Kodas 0', `Skubiai visiems patruliams, vyksta vagystė iš parduotuvės ${shopItem.name}`, player.position, 4);
            dispatcher.sendLocalPos('Kodas 0', `Skubiai visiems patruliams, vyksta vagystė iš parduotuvės ${shopItem.name}`, player.position, 5);
            ...
            setTimeout(function () {
                ...
                try {
                    ...
                    if (player.health < 1) {
                        player.notify('~r~Jus istiko koma, isilaužimas atsauktas.');
                        return;
                    }
                    ...
                    if (methods.getRandomInt(0, 100) < 40) {
                        ...
                        player.notify('~g~Sekmingai isilaužete i kasos aparata');
...
fraction.addMoney = function(id, money, name = "Operacija su sąskaita") {
    fraction.addHistory('Sistema', name, `Įrašyta į biudžetą: ${methods.cryptoFormat(money)}`, id);
    fraction.setMoney(id, fraction.getMoney(id) + methods.parseFloat(money));
};

fraction.removeMoney = function(id, money, name = "Operacija su sąskaita") {
    fraction.addHistory('Sistema', name, `Išleista iš biudžeto: ${methods.cryptoFormat(money * -1)}`, id);
    fraction.setMoney(id, fraction.getMoney(id) - methods.parseFloat(money));
};
...
fraction.editFractionName = function(player, text) {
    ...
    player.notify('~g~Pakeitete savo organizacijos pavadinima');
...
fraction.editFractionLeader = function(player, text) {
    ...
    player.notify('~g~Atnaujinote rango pavadinima');
...
fraction.deleteFractionRank = function(player, rankId, depId) {
    ...
    player.notify('~g~Jus istrynete pranesima');
...
fraction.editFractionDep = function(player, text, depId) {
    ...
    player.notify('~g~Atnaujinote skyriaus pavadinima');
...
fraction.deleteFractionDep = function(player, depId) {
    ...
    player.notify('~g~Istrynete skyriu');
...
fraction.addFractionRank = function(player, text, depId) {
    ...
    player.notify('~g~Pridejote pozicija');
...
fraction.createFractionDep = function(player, text) {
    ...
    let rankList = JSON.parse(fraction.get(id, 'rank_list'));
    rankList.push(["Skyriaus vadovas", "Skyriaus vadovo pavaduotojas"]);

    fraction.set(id, "rank_type_list", JSON.stringify(depList));
    fraction.set(id, "rank_list", JSON.stringify(rankList));

    player.notify('~g~Pakeitete prideta nauja skyriu');
...
fraction.updateOwnerInfo = function (id, userId) {
    ...
    if (userId == 0) {
        fraction.set(id, "rank_leader", 'Lyderis');
        fraction.set(id, "rank_sub_leader", 'Pavaduotojas');
        fraction.set(id, "rank_type_list", '["Pagrindinė sudėtis"]');
        fraction.set(id, "rank_list", '[["Skyriaus vadovas", "Skyriaus vadovo pavaduotojas", "Padėjėjas"]]');
    }
    else
...
fraction.create = function (player, id) {
    ...
    if (user.get(player, 'fraction_id2') > 0) {
        player.notify('~r~Jau esate organizacijos narys');
        return;
    }
    if (fraction.get(id, 'owner_id') > 0) {
        player.notify('~r~Organizacija jau turi savininka');
        return;
    }
    if (user.getCryptoMoney(player) < 500) {
        player.notify('~r~Neturite pakankamai BitCoin valiutos organizacijai sukurti');
        return;
    }
    ...
    fraction.addHistory(user.getRpName(player), 'Įsteigta organizacija', '', id);
    fraction.updateOwnerInfo(id, user.getId(player));
    fraction.set(id, "name", 'Grupavimas');
    user.removeCryptoMoney(player, 500, 'Organizacijos įsteigimas');
    ...
    setTimeout(function () {
        ...
        user.save(player);
        fraction.save(id);
        player.notify('~g~Sveikiname ikurus organizacija');
    }, 500);
};
...
fraction.destroy = function (player, id) {
    ...
    if (!user.isLeader2(player)) {
        player.notify('~r~Si organizacija nesuteiks jums jokiu kreditu');
        return;
    }
...
fraction.destroyJust = function (id) {
    ...
    fraction.set(id, "name", 'Laisvas slotas');
    ...
    mp.players.forEach(p => {
        ...
        if (user.get(p, 'fraction_id2') == id) {
            ...
            p.notify('~y~Organizacija buvo isformuota');
        }
    });
};

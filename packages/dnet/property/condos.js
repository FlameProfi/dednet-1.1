...
condos.loadAll = function() {
    ...
    mysql.executeQuery(`SELECT * FROM condos`, function (err, rows, fields) {
        rows.forEach(function(item) {
            ...
            methods.createCp(hBlip.position.x, hBlip.position.y, hBlip.position.z, "Paspauskite ~g~Е~s~ jog atidarytumėte meniu", 0.8, -1, [33, 150, 243, 100], 0.3);
            hBlip.safe = null;
            hBlip.safeDoor = null;
...
condos.loadLast = function() {
    ...
    mysql.executeQuery(`SELECT * FROM condos ORDER BY id DESC LIMIT 1`, function (err, rows, fields) {
        ...
        rows.forEach(function(item) {
            ...
            let id = item['id'];
            discord.sendMarketProperty(`Butas #${condos.get(id, 'number')}`, `Adresas: ${condos.get(id, 'address')} / ${condos.get(id, 'street')} #${condos.get(id, 'number')}\nValstybinė kaina: ${methods.moneyFormat(condos.get(id, 'price'))}`);

            chat.sendToAll(`Butas pakrautas. ID: ${item['id']}. HID: ${item['condo_big_id']}. Name: ${item['number']}. Int: ${item['interior']}. Price: ${methods.moneyFormat(item['price'])}`);
...
condos.loadBigLast = function() {
    ...
    mysql.executeQuery(`SELECT * FROM condos_big ORDER BY id DESC LIMIT 1`, function (err, rows, fields) {
        ...
        rows.forEach(function(item) {
            ...
            chat.sendToAll(`Butas pakrautas. ID: ${item['id']}. Name: ${item['number']}`);
        });
        count = rows.length;
...
condos.sell = function (player) {
    ...
    if (hInfo.get('user_id') != user.get(player, 'id')) {
        player.notify('~r~Sis butas jums nepriklauso');
        return;
    }
    ...
    user.addMoney(player, nalog, 'Buto pardavimas ' + hInfo.get('address') + ' №' + hInfo.get('number'));

    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        user.addHistory(player, 3, 'Pardavė butą ' + hInfo.get('address') + ' №' + hInfo.get('number') + '. Kaina: ' + methods.moneyFormat(nalog));
        player.notify(`~g~Jus pardavete buta\nMokestis:~s~ ${coffer.getTaxProperty()}%\n~g~Gauta:~s~ ${methods.moneyFormat(nalog)}`);
        user.save(player);
    }, 1000);
};
...
condos.buy = function (player, id) {
    ...
    if (hInfo.get('user_id') > 0) {
        player.notify('~r~Nekilnojamas turtas jau nupirktas');
        return false;
    }
    ...
    user.removeMoney(player, hInfo.get('price'), 'Buto pirkimas ' + hInfo.get('address') + ' №' + hInfo.get('number'));
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        user.addHistory(player, 3, 'Nusipirko butą ' + hInfo.get('address') + ' №' + hInfo.get('number') + '. Kaina: ' + methods.moneyFormat(hInfo.get('price')));
        user.save(player);
        player.notify('~g~Sveikiname nusipirkus buta!');
    }, 500);
    return true;
};

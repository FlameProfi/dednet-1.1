...
yachts.loadAll = function() {
    ...
    mysql.executeQuery(`SELECT * FROM yachts`, function (err, rows, fields) {
        rows.forEach(function(item) {
            ...
            methods.createCp(hBlip.position.x, hBlip.position.y, hBlip.position.z, "Paspauskite ~g~Е~s~ jog atidaerytumėte meniu", 0.8, -1, [33, 150, 243, 100], 0.3);
...
yachts.sell = function (player) {
    ...
    if (hInfo.get('user_id') != user.get(player, 'id')) {
        player.notify('~r~Si jachta jums nepriklauso');
        return;
    }
    ...
    user.addMoney(player, nalog, 'Jachtos pardavimas ' + hInfo.get('name') + ' №' + hInfo.get('id'));

    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        user.addHistory(player, 3, 'Pardavė jachtą ' + hInfo.get('name') + ' №' + hInfo.get('id') + '. Kaina: ' + methods.moneyFormat(nalog));
        player.notify(`~g~Jus pardavete jachta\nMokestis:~s~ ${coffer.getTaxProperty()}%\n~g~Gauta:~s~ ${methods.moneyFormat(nalog)}`);
        user.save(player);
    }, 1000);
};
...
yachts.buy = function (player, id) {
    ...
    user.removeMoney(player, hInfo.get('price'), 'Jachtos pirkimas ' + hInfo.get('name') + ' №' + hInfo.get('id'));
    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        user.addHistory(player, 3, 'Nusipirko jachtą ' + hInfo.get('name') + ' №' + hInfo.get('id') + '. Kaina: ' + methods.moneyFormat(hInfo.get('price')));
        user.save(player);

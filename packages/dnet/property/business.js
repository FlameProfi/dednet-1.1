...
business.addMoney = function(id, money, name = "Operacija su sąskaita") {
...
business.removeMoney = function(id, money, name = "Operacija su sąskaita") {
    if (id === 0 || business.get(id, 'user_id') === 0) {
...
business.updateOwnerInfo = function (bId, userId, userName) {
    ...
    if (userId === 0) {
        discord.sendMarketBusiness(`${business.get(bId, 'name')}`, `Valstybinė kaina: ${methods.moneyFormat(business.get(bId, 'price'))}`);
    }
};
...
business.sell = function (player) {
    ...
    user.addMoney(player, nalog, 'Verslo pardavimas ' + hInfo.get('name'));

    discord.sendMarketBusiness(`${business.get(hInfo.get('id'), 'name')}`, `Valstybinė kaina: ${methods.moneyFormat(business.get(hInfo.get('id'), 'price'))}`);
    ...
    setTimeout(function () {
        ...
        user.addHistory(player, 3, 'Pardavė verslą ' + hInfo.get('name') + ' №' + hInfo.get('id') + '. Kaina: ' + methods.moneyFormat(nalog));
        player.notify(`~g~Jus pardavete versla\nMokestis:~s~ ${coffer.getTaxProperty()}%\n~g~Gauta:~s~ ${methods.moneyFormat(nalog)}`);
        user.save(player);
...
business.buy = function (player, id) {
    ...
    if (user.get(player, 'business_id') > 0) {
        player.notify('~r~Turite nekilnojamojo turto');
        return false;
    }
    ...
    if (!user.get(player, 'biz_lic')) {
        player.notify('~r~Neturite verslo liudijimo\nJi isigyti galima is valstybes pareigunu.');
        return false;
    }
    ...
    user.removeBankMoney(player, hInfo.get('price'), 'Verslo pirkimas ' + hInfo.get('name'));

    setTimeout(function () {
        if (!user.isLogin(player))
            return;
        user.addHistory(player, 3, 'Nusipirko verslą ' + hInfo.get('name') + ' №' + hInfo.get('id') + '. Kaina: ' + methods.moneyFormat(hInfo.get('price')));
        user.save(player);
        player.notify('~g~Sveikiname nusipirkus versla!');
        user.updateClientCache(player);
        business.save(id);
    }, 1000);
    return true;

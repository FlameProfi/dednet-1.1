        ...
        if (info.x == 0) {
            switch (fractionId) {
                case 1:
                    ...
                    break;
                case 2:
                    spawnPos = new mp.Vector3(449.2828369140625, -1017.2554321289062, 27.537166595458984);
                    spawnRot = methods.parseFloat(88.92253875732422);
                    break;
                case 3:
    ...
    try {
        ...
        if (containerId != undefined && vehicle.getVariable('user_id') > 0)
        {
            if (vehicles.get(containerId, 'is_cop_park') > 0) {
                player.notify('~r~Transporto priemone jau stovi konfiskuotoje aiksteleje');
                return;
            }
            ...
            player.notify('~g~Jus gavote premija ' + methods.moneyFormat(price / 2));
        }
...
vehicles.updateOwnerInfo = function (id, userId, userName) {
    ...
    if (userId == 0) {
        ...
        try {
            ...
            discord.sendMarketVehicles(`${vehInfo.display_name}`, `Valstybinė kaina: ${methods.moneyFormat(vehInfo.price)}\nMaks. greitis: ~${vehInfo.sm}км/ч\nKlasė: ${vehInfo.class_name}`, `https://pastaex.fun/client/images/carsv/640/${vehInfo.display_name.toLowerCase()}.jpg`);
        }
        catch (e) {}
    }
    else {
...
vehicles.sell = function (player, slot) {
    ...
    mp.vehicles.forEach(function (veh) {
        ...
        if (veh.getVariable('container') == containerId) {
            ...
            user.addMoney(player, nalog, 'Transporto pardavimas ' + vInfo.get('name'));
            ...
            setTimeout(function () {
                ...
                user.addHistory(player, 3, 'Pardavė transportą ' + vInfo.get('name') + '. Kaina: ' + methods.moneyFormat(nalog));

                if (vInfo.get('s_km') < 300)
                    player.notify(`~g~Jus pardavete transporta\nMokestis:~s~ Nėra\n~g~Gauta:~s~ ${methods.moneyFormat(nalog)}`);
                else
                    player.notify(`~g~Jus pardavete transporta\nMokestis:~s~ ${(coffer.getTaxProperty() + taxOffset)}%\n~g~Gauta:~s~ ${methods.moneyFormat(nalog)}`);
                user.save(player);
            }, 1000);
        }
    });
    ...
    if (!isSpawn)
        player.notify('~r~Pirmiausia isigykite transporto priemone');
};
...
vehicles.addNew = (model, count) => {
    ...
    discord.sendMarketVehicles(`${vInfo.display_name}`, `Valstybinė kaina: ${methods.moneyFormat(vInfo.price)}\nMaks. greitis: ~${vInfo.sm}km/h\nKlasė: ${vInfo.class_name}`, `https://pastaex.fun/client/images/carsv/640/${vInfo.display_name.toLowerCase()}.jpg`);
};
...
vehicles.lockStatus = (player, vehicle) => {
    ...
    try {
        ...
        if (!vehicle.locked)
            player.notify('Jus ~g~atrakinote~s~ transporto priemone');
        else
            player.notify('Jus ~r~užrakinote~s~ transporto priemone');
    ...
    try {
        if (vehicles.getFuel(vehicle) === 0) {
            ...
            if (vInfo.fuel_type !== 3) {
                player.notify('~r~Transporto priemoneje baigesi degalai\nNustatoma degalu papildymo žyma');
        ...
        if (vehicle.broke) {
            player.notify('~r~Transporto priemones variklis neužsiveda, sužinokite, kas gali buti negerai');
            return;
        }
        ...
        if (eStatus && s_elec && s_elec < 30) {
            if (methods.getRandomInt(0, 10) < 5) {
                player.notify('~r~Variklio nepavyko užvesti del elektronikos pažeidimu, bandykite dar karta');
                return;
            }
        }
        vSync.setEngineState(vehicle, eStatus);
        if (eStatus)
            player.notify('Jus ~g~užvedete~s~ varikli');
        else
            player.notify('Jus ~r~užgesinote~s~ varikli');
    }
    catch (e) {
        console.log(e);
    }

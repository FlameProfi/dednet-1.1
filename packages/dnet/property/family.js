...
family.addMoney = function(id, money, name = "Operacija su sąskaita") {
    family.addHistory('Sistema', name, `Įrašyta į biudžetą: ${methods.cryptoFormat(money)}`, id);
    family.setMoney(id, family.getMoney(id) + methods.parseFloat(money));
};

family.removeMoney = function(id, money, name = "Operacija su sąskaita") {
    family.addHistory('Sistema', name, `Išleista iš biudžeto: ${methods.cryptoFormat(money * -1)}`, id);
    family.setMoney(id, family.getMoney(id) - methods.parseFloat(money));
};
...
family.create = function (player, name) {
    ...
    if (user.get(player, 'family_id') > 0) {
        player.notify('~r~Jau esate seimos narys');
        return;
    }
    if (user.getMoney(player) < 500000) {
        player.notify('~r~Neturite pakankamai pinigu seimai sukurti');
        return;
    }

    name = methods.removeQuotes2(methods.removeSpecialChars(methods.removeQuotes(name)));
    if (name === '') {
        player.notify('~r~Seimos pavadinime neturetu buti kabuciu.');
        return;
    }
        ...
        if (rows.length === 0) {
            ...
            setTimeout(function () {
                mysql.executeQuery(`SELECT * FROM family_list WHERE owner_id = '${user.getId(player)}' ORDER BY id DESC`, function (err, rows, fields) {
                    if (rows.length === 0) {
                        player.notify('~r~Seimos sukurimo klaida');
                    }
                    else {
                        let id = rows[0]['id'];
                        family.loadById(id);
                        ...
                        user.removeMoney(player, 500000, 'Šeimos kūrimas ' + name);

                        setTimeout(function () {
                            if (!user.isLogin(player))
                                return;
                            user.save(player);
                            player.notify('~g~Sveikiname sukurus seima');
                        }, 500);
                    }
                });
            }, 500)
            ...
            player.notify('~r~Tokia seima jau egzistuoja');
        }
    });
...
family.destroy = function (player, id) {
    ...
    if (!user.isLeaderF(player)) {
        player.notify('~r~Si seima jums nepades');
        return;
    }

    player.notify('~y~Seima buvo isardyta');
...
family.destroyJust = function (id) {
    ...
    mp.players.forEach(p => {
        ...
        if (user.get(p, 'family_id') == id) {
            ...
            p.notify('~y~Seima buvo isardyta');
        }
    });
